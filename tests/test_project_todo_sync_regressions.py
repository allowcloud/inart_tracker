...
class ProjectTodoSyncRegressionTest(unittest.TestCase):
    ...
    def test_matrix_does_not_treat_scoped_completion_followup_as_project_done(self) -> None:
        ns = load_app_functions(
            "norm_text",
            "has_project_finish_signal",
        ...
        globals_map["get_macro_phase"] = lambda stage, evt="", **kwargs: "工程"
        ...
        self.assertIn("工程拆件", active)
        self.assertNotIn("✅ 已完成(结束)", active)
    ...
    def test_apply_pending_main_navigation_commits_queued_state(self) -> None:
        ...
        fake_state = FakeSessionState(
            _pending_main_nav_menu="📊 全局大盘与甘特图",
            _pending_current_proj_context="1/6马尔福",
            _pending_pm_sel_proj="1/6马尔福",
        )
        ns.apply_pending_main_navigation.__globals__["st"] = types.SimpleNamespace(session_state=fake_state)
