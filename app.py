...
# 🚀 补丁：将指南加入侧边栏导航
menu = st.sidebar.radio("模块导航", [MENU_DASHBOARD, MENU_SPECIFIC, MENU_FASTLOG, MENU_PACKING, MENU_COST, MENU_HISTORY, MENU_GUIDE])
...
# ==========================================
    ...
    st.title("📖 INART PM 系统 (v44) 核心操作指南")
    st.info("👋 欢迎使用全新升级的 INART 产品研发追踪管控系统！本系统专为解决制造业并行研发、跨部门协作痛点而生。只需几分钟，即可掌握核心神器玩法。")

    st.markdown("---")

    with st.expander("🎯 核心场景 1：日常进度交接与流转（PM 必看）", expanded=True):
        st.markdown("""
        当你需要更新某个部件（如：头雕、素体）的研发进度，或者将其流转给下一个环节时：
        1. **进入专属操作台**：点击左侧导航栏的 **【🎯 特定项目管控台】**，在上方选择你需要操作的项目。
        2. **选择要推进的部件**：在下方 **【🔧 进度明细与流转交接工作台】** 中，从下拉框选择你要更新的部件（如不选，默认更新“全局进度”）。
        3. **分发责任人（智能词库）**：点击对应工序的下拉框。如果团队来了新人，拉到最底选择 `➕ 手动输入新成员...`，输入后系统会**自动记住**这个名字，下次可直接模糊搜索。
        4. **填入进展详情**：
           * 选择 **【记录类型】**（正常流转 or 被打回）。
           * 填写具体的交接详情。
           * **💡 关键技巧**：如果这个部件的某个阶段（比如“立项”）已经彻底做完、不再返工，请务必勾选 `✅ 标记该工序已彻底完成`。系统矩阵图中该阶段会由蓝变绿，并为你自动记录工时耗时！
        """)

    with st.expander("🖼️ 核心场景 2：图片上传的“原生免按钮粘贴神技”", expanded=True):
        st.markdown("""
        遇到问题反馈或设计定稿，我们需要上传参考图。系统支持无缝的截图粘贴（`Ctrl+V`）：

        **方法一（常规多图粘贴）**：
        1. 使用截图工具（如微信、Snipaste）截取任意画面。
        2. 点击页面上 **【📋 专属剪贴板捕获区】** 按钮。
        3. 按下键盘 `Ctrl+V`（Mac 为 `Cmd+V`）。
        4. 此时下方会出现带红色 **`🗑️ 移除`** 按钮的缩略图。
        5. **你可以不断截图、不断按 `Ctrl+V`**，图片会自动排成九宫格暂存！如果传错了，点一下垃圾桶即可删掉。

        **方法二（原生闪电粘贴）**：
        想直接把图片贴进下方的虚线框？
        **千万别用鼠标去点虚线框！** 点击网页里的【任意空白背景处】（让网页获得焦点），直接按 `Ctrl+V`，图片就会丝滑飞入虚线框！
        """)

    with st.expander("📊 核心场景 3：全局大盘与自动升级（主管视角）", expanded=False):
        st.markdown("""
        想快速了解所有项目的风险和负荷？
        1. **宏观透视**：点击左侧导航栏的 **【📊 全局大盘与甘特图】**。
        2. **风险颜色预警**：系统会根据设定的“开定时间”和当前进度，自动亮起 🔴逾期预警、🟡研发中、🟢生产期 等状态灯。
        3. **团队负荷看板**：大盘底部会自动拉取目前每个人手头积压的并行任务数量，谁缺人手一目了然。

        **💡 领头羊自动同步引擎**：
        如果在某个项目里，某一部件（如：地台）进度突飞猛进，直接进入了“工程拆件”，你**不需要**手动去改项目的“全局进度”。系统底层会自动感知，并将全局进度**强行对齐拉平**至“工程拆件”，并在历史记录中留下 `[系统自动追踪]` 的流转日志。
        """)

    with st.expander("🔍 核心场景 4：手滑填错了怎么办？（历史补救与词库清洗）", expanded=False):
        st.markdown("""
        **状况 A：发现系统里某个前同事离职了，或者某个名字打错了（比如 Mo 打成了 Moo）。**
        * **解法**：在【特定项目管控台】中，点开 `🛠️ 人员词库管理 (修正错别字 / 移除人员)` 的折叠面板。左边选错的名字，右边填对的名字（填空代表全库移除），点击确认，系统瞬间完成全库历史修正！

        **状况 B：刚才保存的一条交接记录，发现阶段选错了，或者文字描述有语病。**
        * **解法**：点击左侧导航栏的 **【🔍 历史溯源 (全局可编)】**。找到那条记录，**双击表格里的文字或下拉框**直接修改！改完后点击上方的 `💾 确认并覆盖保存历史记录` 即可无痕修复。*(注：选中整行按 `Delete` 键可直接删除该条记录)*。
        """)

    with st.expander("⚙️ 核心场景 5：数据安全与备份（管理员专属）", expanded=False):
        st.markdown("""
        为了防止意外，请定期进行系统数据备份：
        1. 看向网页最左侧边栏的底部：**【⚙️ 数据备份与恢复】**。
        2. 点击 **`💾 下载最新系统数据 (备份)`**，将下载的 `.json` 档案妥善保存到本地或网盘。
        3. 如果发生严重的数据破坏（如某人恶意清空了某个项目），将备份的 `.json` 拖入下方的上传框，点击红色确认按钮即可时光倒流！

        *(⚠️警告：恢复数据会覆盖掉当前在线的所有新进度，请谨慎操作！)*
        """)

    st.divider()
    st.caption("💡 如果点击保存后页面似乎没反应，请看网页右上角。若出现 **Running...**，说明系统正在处理运算，请耐心等待 1-2 秒！")
