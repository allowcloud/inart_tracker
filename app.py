...
def _build_project_attention_map_cached(project_names_key):
    names = [str(x).strip() for x in (project_names_key or ()) if str(x).strip() and str(x).strip() != "系统配置"]
    attention = {}
    ...
    return {k: tuple(dict.fromkeys(v)) for k, v in attention.items()}


def build_project_attention_map(project_names):
    names = tuple(
        str(x).strip()
        for x in (project_names or [])
        if str(x).strip() and str(x).strip() != "系统配置"
    )
    cached = _build_project_attention_map_cached(names)
...
def collect_stage_activity(raw_logs, stages):
    ...
    if any(stg in active_stages or stg in completed_stages for stg in guantu_related):
        for pre_stage in ["建模(含打印/签样)", "设计", "工程拆件", "手板/结构板"]:
...
def clear_todo_quick_add_form_state(state_obj, preserve_scope=True):
    ...
    if not isinstance(state_obj, dict):
        state["_todo_quick_add_clear_request"] = {"preserve_scope": bool(preserve_scope)}
        return

    state["todo_title_global"] = ""
    state["todo_cpddl_global"] = ""
    state["todo_ref_global_multi"] = []
    state["todo_people_global"] = []
    state["todo_new_proj_name"] = ""
    state["todo_new_person_role"] = ""
    state["todo_new_person_name"] = ""
    state["todo_global_autofill_preview"] = {}
    state["todo_global_autofill_sig"] = ""

    for key in ["todo_new_proj_owner", "todo_new_proj_ratio", "todo_new_proj_ip_owner"]:
        try:
            state.pop(key, None)
        except Exception:
            pass
    ...
    if not preserve_scope:
        try:
            state.pop("todo_scope_global", None)
        except Exception:
            pass
    return True


def apply_todo_quick_add_form_state_clear_request(state_obj):
    state = state_obj if hasattr(state_obj, "get") else {}
    clear_req = state.get("_todo_quick_add_clear_request")
    if not isinstance(clear_req, dict):
        return False
    preserve_scope = bool(clear_req.get("preserve_scope", True))
    state["todo_title_global"] = ""
    state["todo_cpddl_global"] = ""
    state["todo_ref_global_multi"] = []
    state["todo_people_global"] = []
    ...
    if not preserve_scope:
        try:
            ...
            pass
    try:
        state.pop("_todo_quick_add_clear_request", None)
    except Exception:
        pass
    return True
...
def build_todo_quick_add_entry(
    ...
    if str(temporal.get("route", "")).strip() == "past":
        ...
        if apply_side_effects and append_todo_completion_history(new_td, done_day):
...
def todo_manager_norm_signature(todo_rows, valid_projs=None):
    ...
    parts = [f"valid:{'|'.join(valid_list)}"]
    for td in rows:
        if not isinstance(td, dict):
            continue
        proj_list = td.get("关联项目列表", [])
        if not isinstance(proj_list, list):
            proj_list = normalize_todo_project_list(proj_list)
        proj_text = ",".join([str(x).strip() for x in proj_list if str(x).strip()])
        parts.append("||".join([
            str(td.get("_id", "")).strip(),
            str(td.get("任务", "")).strip(),
            str(td.get("CPDDL", "")).strip(),
            str(td.get("关联项目", "")).strip(),
            proj_text,
            str(td.get("关联人员", "")).strip(),
            str(td.get("所属视角", "")).strip(),
            str(td.get("创建者视角", "")).strip(),
            "1" if bool(td.get("完成")) else "0",
            str(td.get("完成时间", "")).strip(),
        ]))
    return "\n".join(parts)
...
def infer_default_stage_from_project_milestone(proj_data):
    ...
    if ms in ["待立项", "待开定", "已开定"]:
        for cand in ["预研", "建模(含打印/签样)", "工程拆件", "设计"]:
...
def infer_latest_component_stage_from_logs(project_name, component_name):
    proj = db.get(str(project_name or "").strip(), {})
    comps = proj.get("部件列表", {}) if isinstance(proj, dict) else {}
    comp = comps.get(str(component_name or "").strip(), {}) if isinstance(comps, dict) else {}
    raw_logs = comp.get("日志流", []) if isinstance(comp, dict) else []
    parsed = []
    for idx, lg in enumerate(raw_logs):
        if not isinstance(lg, dict) or is_hidden_system_log(lg):
            continue
        dt = parse_date_safe(lg.get("日期", ""))
        stg = str(lg.get("工序", "")).strip()
        if dt and stg in STAGES_UNIFIED:
            parsed.append((dt, idx, stg))
    parsed.sort(key=lambda x: (x[0], x[1]), reverse=True)
    for _, _, stg in parsed:
        if stg != "✅ 已完成(结束)" and not is_pause_stage(stg):
            return stg
    return ""


def infer_default_handoff_stage(project_name, component_name="全局进度"):
    proj = str(project_name or "").strip()
    comp_name = str(component_name or "").strip() or "全局进度"
    proj_data = db.get(proj, {}) if proj in db else {}
    comps = proj_data.get("部件列表", {}) if isinstance(proj_data, dict) else {}
    global_key = next((k for k in comps.keys() if "全局" in str(k)), "全局进度") if isinstance(comps, dict) else "全局进度"
    comp_data = comps.get(comp_name, {}) if isinstance(comps, dict) else {}
    global_data = comps.get(global_key, {}) if isinstance(comps, dict) else {}

    candidates = []
    for src in [comp_data, global_data]:
        stg = str((src or {}).get("主流程", "")).strip()
        if stg in STAGES_UNIFIED and stg != "✅ 已完成(结束)" and not is_pause_stage(stg):
            candidates.append(stg)

    for name in [comp_name, global_key]:
        lg_stage = infer_latest_component_stage_from_logs(proj, name)
        if lg_stage:
            candidates.append(lg_stage)

    exp_stage = str(build_project_current_explanation(proj).get("阶段", "")).strip()
    if exp_stage in STAGES_UNIFIED and exp_stage != "✅ 已完成(结束)" and not is_pause_stage(exp_stage):
        candidates.append(exp_stage)

    fallback = infer_default_stage_from_project_milestone(proj_data)
    if fallback in STAGES_UNIFIED:
        candidates.append(fallback)

    ranked = [c for c in candidates if c in STAGES_UNIFIED]
    if not ranked:
        return STAGES_UNIFIED[0] if STAGES_UNIFIED else "立项"
    return max(ranked, key=lambda s: get_stage_index(s, STAGES_UNIFIED))


def infer_file_handoff_transition_label(text, target_stage=""):
    s = norm_text(text)
    tgt = str(target_stage or "").strip()
    if not s:
        return ""
    if any(sig in s for sig in ["设计将文件转交给工程", "设计交接工程", "转交给工程", "下发工程", "发给工程", "交给工程", "工程接收", "工程已接收", "工程开始"]):
        return "设计→工程"
    if any(sig in s for sig in ["建模文件转交设计", "建模转设计", "建模交接设计", "模型转设计", "模型交接设计", "发给设计", "交给设计"]) and any(tok in s for tok in ["建模", "模型", "头雕", "植发", "雕"]):
        return "建模→设计"
    if any(sig in s for sig in ["工程回设计", "工程转设计", "回设计复核", "给设计复核", "给设计确认"]) and "工程" in s:
        return "工程→设计"
    if tgt == "设计" and any(tok in s for tok in ["建模", "模型", "头雕", "植发", "雕"]):
        return "建模→设计"
    if tgt == "工程拆件" and any(tok in s for tok in ["设计", "官图", "排版"]):
        return "设计→工程"
    return ""
...
def clamp_timeline_date(date_obj, ref_date=None):
    dt = date_obj if isinstance(date_obj, datetime.date) else None
    if not dt:
        return None
    today = ref_date if isinstance(ref_date, datetime.date) else datetime.date.today()
    return today if dt > today else dt


def format_gap_days_text(latest_date, ref_date=None):
    dt = clamp_timeline_date(latest_date, ref_date=ref_date)
    if not isinstance(dt, datetime.date):
        return "-"
    today = ref_date if isinstance(ref_date, datetime.date) else datetime.date.today()
    return f"{max((today - dt).days, 0)} 天"
...
def resolve_todo_completion_event_day(title="", cpddl_text="", temporal_info=None, ref_date=None):
    ...
    temporal = temporal_info if isinstance(temporal_info, dict) else classify_temporal_event_route(
        f"{str(title or '').strip()} {str(cpddl_text or '').strip()}".strip(),
        ref_date=ref,
        prefer_past=False,
    )
    route = str((temporal or {}).get("route", "")).strip()
    evt_date = temporal.get("date") if isinstance((temporal or {}).get("date"), datetime.date) else None
    if route == "past" and isinstance(evt_date, datetime.date):
        return evt_date
    return ref
...
def refine_dashboard_todo_task_text(raw_text, task_text="", due_dt=None, component_name="", stage_name="", component_labels=None):
    ...
    def _ordered_component_labels():
        labels = []
        for item in (component_labels or []):
            label = str(item or "").strip(" ，,;；|")
            if not label:
                continue
            if label in ["全局", "整体", "项目", "打印件", "件", "版本", "版"]:
                continue
            if label not in labels:
                labels.append(label)
        return labels

    def _fallback_component_label():
        comp_txt = str(component_name or "").strip()
        if not comp_txt or comp_txt == "全局进度":
            return ""
        if "头雕" in comp_txt or "表情" in comp_txt:
            return "头雕"
        return re.sub(r"\(.*?\)", "", comp_txt).strip()

    def _compose_component_label():
        labels = _ordered_component_labels()
        if labels:
            return "、".join(labels)
        return _fallback_component_label()
    ...
    print_like_tokens = ["打印", "打印件", "安排打印", "已安排打印", "内部打印", "打样件"]
    arrival_like_tokens = ["待收件", "待回件", "到件", "到货", "预计到", "收件", "回件"]
    expected_arrival_signal = bool(
        re.search(r"预计\s*(?:\d{1,2}[/-]\d{1,2}|今天|明天|后天)?\s*(?:到|到件|到货|收件)", raw)
    )
    if (
        isinstance(due_dt, datetime.date)
        and any(tok in raw for tok in print_like_tokens)
        and (expected_arrival_signal or any(tok in raw for tok in arrival_like_tokens) or task in {"到", "到件", "到货"})
    ):
        label = _compose_component_label()
        if label:
            return f"{label}待收件"
        return "待收件"

    if task in {"到", "到件", "到货"} and isinstance(due_dt, datetime.date):
        label = _compose_component_label()
        if label:
            return f"{label}待收件"
        return "待收件"
    ...
    for seg in segments:
        ...
        for tag in explicit_tags:
            ...
            if comp_candidate and not component_name:
                ...
                component_candidates = [comp_candidate]
        ...
        if not component_name or not stage_name:
            ...
            if hint_rows:
                ...
                if "🌐 全局进度 (Overall)" in comp_list and len(comp_list) > 1:
                    comp_list = [x for x in comp_list if x != "🌐 全局进度 (Overall)"]
                raw_component_labels = first_hint.get("component_labels", {}) or {}
                component_label_map = {
                    ("全局进度" if str(comp_key).strip() == "🌐 全局进度 (Overall)" else str(comp_key).strip()): [
                        str(label).strip() for label in (label_list or []) if str(label).strip()
                    ]
                    for comp_key, label_list in raw_component_labels.items()
                    if str(comp_key).strip()
                }
                if not component_name and comp_list:
                    component_name = "全局进度" if comp_list[0] == "🌐 全局进度 (Overall)" else comp_list[0]
                if comp_list:
                    component_candidates = [
                        "全局进度" if comp_txt == "🌐 全局进度 (Overall)" else comp_txt
                        for comp_txt in comp_list
                    ]
                if not stage_name:
                    stage_name = str(first_hint.get("stage", "")).strip()

        if (not component_name or not stage_name or not component_candidates) and proj:
            tmp_td = {
                "任务": task,
            ...
            if isinstance(prefill, dict):
                if not component_name:
                    ...
                    if comp_list:
                        ...
                        component_candidates = comp_list
                if not stage_name:
                    stage_name = str(prefill.get("阶段", "")).strip()
        ...
        if not component_candidates:
            component_candidates = [component_name] if component_name else []
        component_candidates = list(dict.fromkeys([
            "全局进度" if str(comp).strip() == "🌐 全局进度 (Overall)" else str(comp).strip()
            for comp in component_candidates
            if str(comp).strip()
        ]))
        if "全局进度" in component_candidates and len(component_candidates) > 1:
            component_candidates = [x for x in component_candidates if x != "全局进度"]

        task_refiner = globals().get("refine_dashboard_todo_task_text")
        row_candidates = component_candidates or [component_name] or [""]
        for comp_candidate in row_candidates:
            row_component = str(comp_candidate or component_name or "").strip()
            row_labels = [str(x).strip() for x in (component_label_map.get(row_component, []) or []) if str(x).strip()]
            row_task = task
            if callable(task_refiner):
                row_task = task_refiner(
                    seg,
                    task_text=task,
                    due_dt=due_dt if isinstance(due_dt, datetime.date) else None,
                    component_name=row_component,
                    stage_name=stage_name,
                    component_labels=row_labels,
                )
            out.append({
                "raw": seg,
                "task": row_task,
...
def expand_dashboard_dynamic_history_entries(text, project_name="", ref_date=None, fallback_component="", fallback_stage=""):
    raw_text = str(text or "").strip()
    if not raw_text:
        return []
    ...
    fallback_comp = str(fallback_component or "").strip() or "全局进度"
    fallback_stage_name = str(fallback_stage or "").strip()
    out = []
    for seg in segments:
        hint = hint_map.get(seg, {})
        seg_comps = [str(x).strip() for x in (hint.get("components", []) or []) if str(x).strip()]
        seg_comps = ["全局进度" if comp_txt == "🌐 全局进度 (Overall)" else comp_txt for comp_txt in seg_comps]
        if "全局进度" in seg_comps and len(seg_comps) > 1:
        ...
        for comp_name in list(dict.fromkeys(seg_comps)):
            out.append({
                ...
                "stage": seg_stage or fallback_stage_name,
            })
    return out
...
def upsert_todo_from_event_text(project_name, event_text, forced_due_dt=None, forced_task_body="", people_text="", actor="系统", scope_override="", prefer_existing_pending=False, return_payload=False, allow_empty_due=False, forced_component="", forced_stage="", fallback_project_people=True):
    ...
    def _pack_result(status, todo_obj):
        td_obj = todo_obj if isinstance(todo_obj, dict) else {}
        todo_id = str(td_obj.get("_id", "")).strip()
        return {
            "status": str(status or "").strip(),
            "todo_id": todo_id,
            "todo_ids": [todo_id] if todo_id else [],
        }
    ...
    if (hit is None) and prefer_existing_pending:
        pending_hits = [
            td for td in todo_all
            if isinstance(td, dict) and (not bool(td.get("完成"))) and todo_matches_project(td, proj)
        ]
    ...
    if hit:
        ...
        if new_proj_list != old_proj_list:
            ...
            changed = True
        payload = _pack_result("updated" if changed else "exists", hit)
...
def unpack_todo_upsert_result(result):
    if isinstance(result, dict):
        status = str(result.get("status", "")).strip()
        todo_ids = [str(x).strip() for x in (result.get("todo_ids", []) or []) if str(x).strip()]
        todo_id = str(result.get("todo_id", "")).strip()
        if todo_id and todo_id not in todo_ids:
            todo_ids.append(todo_id)
        return status, todo_ids
...
def infer_todo_form_defaults(task_text, cpddl_text, valid_projs, current_people_text="", include_people=True, role_person_bundle=None):
    ...
    people_bundle = {"labels": [], "ambiguous": [], "unknown": []}
    inferred_people = []
    if include_people:
        safe_infer_fn = globals().get("safe_infer_todo_people_bundle")
        if callable(safe_infer_fn):
            people_bundle = safe_infer_fn(td_probe, options_bundle=role_person_bundle)
        else:
            try:
                people_bundle = infer_todo_people_bundle(td_probe, options_bundle=role_person_bundle)
            except TypeError:
...
def safe_infer_todo_people_bundle(td, options_bundle=None):
    infer_fn = globals().get("infer_todo_people_bundle")
    if not callable(infer_fn):
        return {"labels": [], "ambiguous": [], "unknown": []}
    if options_bundle is not None:
        try:
            return infer_fn(td, options_bundle=options_bundle)
        except TypeError:
            pass
    return infer_fn(td)
...
def todo_link_status_text(td):
    ...
    if not mod and not dt:
        fallback_evt = get_latest_todo_landing_event(td_obj)
        if isinstance(fallback_evt, dict):
            mod = todo_link_module_label(fallback_evt.get("来源", ""), fallback_evt.get("动作", ""))
            dt = str(fallback_evt.get("日期", "")).strip()
            comp = str(fallback_evt.get("部件", "")).strip()
            stage = str(fallback_evt.get("阶段", "")).strip()
        if not mod and not dt:
            return "未落地"
    parts = []
    if dt:
...
def canonicalize_all_project_references():
    ...
    if isinstance(print_rows, list):
        ...
        for row in print_rows:
            if not isinstance(row, dict):
                continue
            canon_proj = canonicalize_project_name(row.get("项目", ""), valid_projs=valid_projs, alias_map=alias_map)
            if canon_proj != str(row.get("项目", "")).strip():
                changed = True
            row["项目"] = canon_proj
            if canon_proj:
...
def auto_cleanup_project_shells():
    ...
    for proj in [p for p in list(db.keys()) if p != "系统配置"]:
        if is_invalid_project_name(proj):
            if delete_project_and_refs(proj):
                changed = True
    for proj in [p for p in list(db.keys()) if p != "系统配置"]:
        inferred = infer_malformed_ratio_project_target(proj)
        if inferred and inferred != proj:
            if merge_project_into_target(proj, inferred, learned_aliases=[proj]):
                changed = True
    return changed
...
def infer_todo_people_bundle(td, options_bundle=None):
    td_obj = td or {}
    if isinstance(options_bundle, dict):
        labels = list(options_bundle.get("labels", []) or [])
        raw_name_map = options_bundle.get("name_map", {}) or {}
        name_map = {
            str(k): {
                "display": str((v or {}).get("display", "")).strip(),
                "labels": list((v or {}).get("labels", []) or []),
            }
...
def format_todo_people_hint(td, options_bundle=None):
    safe_infer_fn = globals().get("safe_infer_todo_people_bundle")
    if callable(safe_infer_fn):
        bundle = safe_infer_fn(td, options_bundle=options_bundle)
    else:
        try:
            bundle = infer_todo_people_bundle(td, options_bundle=options_bundle)
        except TypeError:
...
def extract_todo_segment_hints(text, project_components=None, comp_kw=None, stage_kw_map=None):
    ...
    def _append_component_label(label_map, comp_name, label_text):
        ...
        if not comp_txt or not label:
            return
        if len(label) < 2:
            return
        label_map.setdefault(comp_txt, [])
        if label not in label_map[comp_txt]:
            label_map[comp_txt].append(label)
    ...
    for seg in segments:
        ...
        for comp in proj_comps:
            ...
            if any(v and ((v in seg) or (norm_text(v) in seg_norm)) for v in variants):
                ...
                for variant in variants:
                    variant_txt = str(variant or "").strip()
                    if variant_txt and ((variant_txt in seg) or (norm_text(variant_txt) in seg_norm)):
                        _append_component_label(seg_component_labels, picked, variant_txt)
        ...
        for kw, target in _sorted_kw_items(comp_kw):
            ...
            _append_component_label(seg_component_labels, picked_comp, kw)

        version_match = re.search(r"(?i)(v\d+\s*版?|第[一二三四五六七八九十\d]+版)", seg)
        if (
            (not seg_comps)
            and version_match
            and any(tok in seg for tok in ["打印", "打印件", "给", "老师", "建模", "修改", "件不对"])
        ):
            picked_head = _pick_component_from_hint("头雕") or "头雕(表情)"
            seg_comps.append(picked_head)
            _append_component_label(seg_component_labels, picked_head, str(version_match.group(1) or "").strip())
        ...
        if seg_comps or stage_name:
            hint_rows.append({
                ...
                "component_labels": seg_component_labels,
                "stage": stage_name,
            })
    return hint_rows
...
def get_explanation_source_weight(source_module=""):
    ...
    return source_weight.get(src, 0)
...
def get_latest_project_log_binding(project_name):
    ...
    latest = None
    for comp_name, comp_info in db.get(proj, {}).get("部件列表", {}).items():
        for log_idx, lg in enumerate(comp_info.get("日志流", [])):
            if is_hidden_system_log(lg):
                continue
            d_txt = str((lg or {}).get("日期", "")).strip()
            d_obj = parse_date_safe(d_txt) or datetime.date.min
            rank_date = clamp_timeline_date(d_obj) or d_obj
            ...
            rank = (rank_date.toordinal(), attention, 0, write_ts, log_idx, str((lg or {}).get("_id", "")), str(comp_name))
            if (latest is None) or (rank > latest["rank"]):
                latest = {"rank": rank, "component": str(comp_name), "log": lg}
    return latest


def collect_latest_dashboard_dynamic_group_logs(project_name, anchor_binding=None):
    proj = str(project_name or "").strip()
    if (not proj) or proj not in db:
        return []

    binding = anchor_binding if isinstance(anchor_binding, dict) else get_latest_project_log_binding(proj)
    if not binding or not isinstance(binding.get("log"), dict):
        return []

    anchor_log = binding.get("log", {}) or {}
    anchor_flow = str(anchor_log.get("流转", "")).strip()
    if anchor_flow != "大盘动态":
        return []

    anchor_date = str(anchor_log.get("日期", "")).strip()
    anchor_write = str(anchor_log.get("写入时间", "")).strip()
    anchor_id = str(anchor_log.get("_id", "")).strip()
    rows = []
    for comp_name, comp_info in db.get(proj, {}).get("部件列表", {}).items():
        for log_idx, lg in enumerate(comp_info.get("日志流", [])):
            if is_hidden_system_log(lg):
                continue
            if str((lg or {}).get("流转", "")).strip() != anchor_flow:
                continue
            if str((lg or {}).get("日期", "")).strip() != anchor_date:
                continue
            log_write = str((lg or {}).get("写入时间", "")).strip()
            log_id = str((lg or {}).get("_id", "")).strip()
            if anchor_write:
                if log_write != anchor_write:
                    continue
            elif anchor_id and log_id != anchor_id:
                continue

            clean = str((lg or {}).get("事件", "")).strip()
            if "补充:" in clean:
                clean = clean.split("补充:")[-1]
            if "】" in clean:
                clean = clean.split("】")[-1]
            clean = clean.split("[系统]")[0].strip() or str((lg or {}).get("事件", "")).strip()
            rows.append({
                "component": str(comp_name).strip() or "全局进度",
                "stage": str((lg or {}).get("工序", "")).strip() or "-",
                "text": clean,
                "date": anchor_date,
                "write_time": log_write,
                "split_index": int((lg or {}).get("拆分序号", 9999) or 9999),
                "log_index": log_idx,
                "_id": log_id,
            })

    rows.sort(key=lambda row: (int(row.get("split_index", 9999) or 9999), str(row.get("component", "")), int(row.get("log_index", 0) or 0)))
    seen = set()
    out = []
    for row in rows:
        key = (str(row.get("component", "")), str(row.get("text", "")))
        if key in seen:
            continue
        seen.add(key)
        out.append(row)
...
def get_latest_standard_event_binding(project_name):
    ...
    for evt in events:
        ...
        rank_date = clamp_timeline_date(evt_date) or evt_date
        ...
        rank = (
            rank_date.toordinal(),
            attention,
            get_explanation_source_weight(src),
...
def todo_link_module_label(source_module="", action_type=""):
    ...
    if src == "PM工作台" and act in ["工作台记录", "工作台联动待办"]:
        return "交接工作台"
    return src


def sync_todo_recent_linkage(todo_ids, source_module="", action_type="", project_name="", component_name="", stage_name="", event_date=None, write_time=""):
    id_list = [str(x).strip() for x in (todo_ids or []) if str(x).strip()]
    if not id_list:
        return 0
    link_module = todo_link_module_label(source_module, action_type)
    link_date = str(event_date) if isinstance(event_date, datetime.date) else str(event_date or "").strip()
    if not link_module and not link_date:
        return 0

    valid_projs = [p for p in db.keys() if p != "系统配置"] if isinstance(db, dict) else []
    alias_map = db.get("系统配置", {}).get("项目别名", {}) if isinstance(db, dict) else {}
    proj = canonicalize_project_name(project_name, valid_projs=valid_projs, alias_map=alias_map) or str(project_name or "").strip()
    comp = _normalize_standard_event_component(component_name)
    stage = str(stage_name or "").strip()
    write_ts = str(write_time or "").strip() or datetime.datetime.now().isoformat(timespec="seconds")

    todo_all = db.setdefault("系统配置", {}).setdefault("PM_TODO_LIST", [])
    touched = 0
    id_set = set(id_list)
    for td in todo_all:
        if str((td or {}).get("_id", "")).strip() not in id_set:
            continue
        td["最近联动模块"] = link_module
        td["最近联动日期"] = link_date
        td["最近联动项目"] = proj
        td["最近联动部件"] = comp
        td["最近联动阶段"] = stage
        td["最近联动写入时间"] = write_ts
        touched += 1
    return touched


def get_latest_todo_landing_event(td_obj):
    todo_id = str((td_obj or {}).get("_id", "")).strip()
    if not todo_id:
        return None
    ...
    for evt in events:
        evt_obj = evt if isinstance(evt, dict) else {}
        evt_todo_ids = [str(x).strip() for x in (evt_obj.get("关联待办", []) or []) if str(x).strip()]
        if todo_id not in evt_todo_ids:
            continue
        if str(evt_obj.get("来源", "")).strip() == "To-do":
            continue
        evt_date_txt = str(evt_obj.get("日期", "")).strip()
        evt_date = parse_date_safe(evt_date_txt) or datetime.date.min
        rank_date = clamp_timeline_date(evt_date) or evt_date
        src = str(evt_obj.get("来源", "")).strip()
        rank = (
            rank_date.toordinal(),
            get_explanation_source_weight(src),
            str(evt_obj.get("写入时间", "")).strip(),
            str(evt_obj.get("_id", "")).strip(),
        )
        if (latest is None) or (rank > latest["rank"]):
            latest = {"rank": rank, "event": evt_obj}
...
def get_latest_live_todo_binding(project_name, pm_view=None):
    proj = str(project_name or "").strip()
    if not proj:
        return None
    alias_map = db.get("系统配置", {}).get("项目别名", {})
...
def build_project_todo_reminder(todo_event=None, live_todo_binding=None, pm_view=None, project_name=""):
    ...
    evt_visible = True
    if pm_view and evt_scope and evt_scope != "所有人":
        evt_visible = (evt_scope == "未分配") if pm_view == "所有人" else (evt_scope == pm_view)
    if live_td and pm_view and (not todo_visible_for_view(live_td, pm_view)):
        live_td = {}
        live_mode = ""
    ...
    if live_td:
        ...
        if task:
            ...
            if bool(live_td.get("完成")) or live_mode == "completed":
                ...
                if not done_txt:
                    done_dt = todo_due_date(live_td)
                    done_txt = str(done_dt) if isinstance(done_dt, datetime.date) else ""
                return {
                    "text": task,
    ...
    if evt_visible and evt_action == "待办完成":
        text = str(evt.get("内容", "")).strip() or str(evt.get("原始文本", "")).strip()
        if text:
            return {
                "text": text,
...
def format_todo_reminder_label(reminder_text, reminder_date="", reminder_action=""):
    text = str(reminder_text or "").strip()
    if not text:
        return ""
    action = str(reminder_action or "").strip()
    if action == "待办完成":
        action_label = "完成待办"
    elif action == "待办重开":
        action_label = "重开待办"
        ...
        action_label = "待办"
    date_txt = str(reminder_date or "").strip()
    date_obj = parse_date_safe(date_txt)
    if isinstance(date_obj, datetime.date):
        date_label = f"{date_obj.month}/{date_obj.day}"
    else:
        date_label = date_txt
    prefix = f"{date_label}{action_label}" if date_label else action_label
    return f"{prefix}：{text}"


def build_explanation_identity(explanation_obj):
    exp = explanation_obj if isinstance(explanation_obj, dict) else {}
    return {
        "text": normalize_linkage_match_text(exp.get("原始文本", "") or exp.get("内容", "")),
        "component": _normalize_standard_event_component(exp.get("部件", "")),
        "stage": str(exp.get("阶段", "")).strip(),
    }


def same_event_match_score(left_explanation, right_explanation):
    left = build_explanation_identity(left_explanation)
    right = build_explanation_identity(right_explanation)
    ...
    if left_text == right_text:
        score += 4
    elif len(left_text) >= 6 and len(right_text) >= 6 and (left_text in right_text or right_text in left_text):
        score += 3

    left_comp = str(left.get("component", "")).strip()
    right_comp = str(right.get("component", "")).strip()
    if left_comp and right_comp:
        if left_comp == right_comp:
            score += 1
        elif "全局" in left_comp or "全局" in right_comp:
            score += 0

    left_stage = str(left.get("stage", "")).strip()
    right_stage = str(right.get("stage", "")).strip()
    if left_stage and right_stage:
        if left_stage == right_stage:
            score += 1
        elif left_stage == "-" or right_stage == "-":
            score += 0

    return score


def normalize_linkage_match_text(text):
    raw = str(text or "").strip()
    if not raw:
        return ""
    ...
    normalized = re.sub(r"CP/DDL[:：][^|｜]+", " ", normalized, flags=re.I)
    normalized = re.sub(r"人员[:：][^|｜]+", " ", normalized)
    normalized = re.sub(r"(?:20\d{2}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}[./月-]\d{1,2})", " ", normalized)
    normalized = re.sub(r"(完成待办|重开待办|待办提醒|待办完成|待办重开|待办)", " ", normalized)
    normalized = re.sub(
        r"待[A-Za-z0-9_\-\u4e00-\u9fff]{1,8}(拆件|打样|确认|反馈|修改|回复|处理|跟进|跟催|收件|回件|打印|安排|提审|出图|建模|翻模|植发|烫色)",
        r"待\1",
        normalized,
    )
    normalized = re.sub(
        r"(?:已)?(?:转交|交给|发给|给)[A-Za-z0-9_\-\u4e00-\u9fff]{1,8}(待(?:打样|确认|反馈|修改|回复|处理|跟进|跟催|收件|回件|打印))",
        r"\1",
        normalized,
    )
    normalizer = globals().get("clean_auto_todo_task_text")
    ...
    return norm_text(normalized)


def should_promote_todo_explanation(current_explanation, todo_explanation):
    current = current_explanation if isinstance(current_explanation, dict) else {}
    todo_exp = todo_explanation if isinstance(todo_explanation, dict) else {}

    todo_text = normalize_linkage_match_text(todo_exp.get("原始文本", "") or todo_exp.get("内容", ""))
    if not todo_text:
        return False

    current_text = normalize_linkage_match_text(current.get("原始文本", "") or current.get("内容", ""))
    if not current_text or current_text == "无数据":
        return True

    current_source = str(current.get("来源", "")).strip()
    current_action = str(current.get("动作", "")).strip()
    if current_source == "To-do" or current_action in ["待办提醒", "待办新建", "待办更新", "待办重开", "待办完成"]:
        return True
...
def build_todo_explanation_snapshot(project_name, todo_binding=None, live_todo=None, todo_reminder=None):
    proj = str(project_name or "").strip()
    evt = todo_binding if isinstance(todo_binding, dict) else {}
    live_td = live_todo if isinstance(live_todo, dict) else {}
    reminder = todo_reminder if isinstance(todo_reminder, dict) else {}
    reminder_text = str(reminder.get("text", "")).strip()
    if not reminder_text:
        return {}

    content = reminder_text or str(evt.get("内容", "")).strip() or str(evt.get("原始文本", "")).strip() or str(live_td.get("任务", "")).strip() or todo_cpddl_text(live_td) or "无数据"
    raw_text = str(evt.get("原始文本", "")).strip() or todo_cpddl_text(live_td) or content
    event_comp = str(reminder.get("component", "")).strip() or str(evt.get("部件", "")).strip()
    event_stage = str(reminder.get("stage", "")).strip() or str(evt.get("阶段", "")).strip()
    event_people = normalize_people_text(live_td.get("关联人员", "")) or str(evt.get("关联人员", "")).strip()
    event_date = str(evt.get("日期", "")).strip()
    if not event_date:
        if bool(live_td.get("完成")) and str(live_td.get("完成时间", "")).strip():
            event_date = str(live_td.get("完成时间", "")).strip()
        else:
            created_dt = parse_date_safe(str(live_td.get("创建", "")).strip())
            event_date = str(created_dt) if isinstance(created_dt, datetime.date) else ""

    if live_td and proj and proj in db:
        try:
            prefill = infer_todo_handoff_prefill(live_td, proj)
        except Exception:
            prefill = {}
        if isinstance(prefill, dict):
            comp_list = [str(x).strip() for x in (prefill.get("部件", []) or []) if str(x).strip()]
            if comp_list and not event_comp:
                event_comp = _normalize_standard_event_component(comp_list[0])
            if not event_stage:
                event_stage = str(prefill.get("阶段", "")).strip()

    todo_ids = [str(x).strip() for x in (evt.get("关联待办", []) or []) if str(x).strip()]
    live_id = str(live_td.get("_id", "")).strip()
    if live_id and live_id not in todo_ids:
        todo_ids.append(live_id)

    action = str(reminder.get("action", "")).strip() or str(evt.get("动作", "")).strip() or "待办提醒"
    return {
        ...
        "关联人员": event_people,
        "关联待办": todo_ids,
        "意图": "past" if action == "待办完成" else "todo",
        "提醒内容": reminder_text,
        "提醒日期": str(reminder.get("date", "")).strip(),
        "提醒动作": action,
        "提醒部件": str(reminder.get("component", "")).strip() or event_comp,
        "提醒阶段": str(reminder.get("stage", "")).strip() or event_stage,
        "模式": "待办提醒",
    }
...
def build_dashboard_dynamic_display(project_name, explanation=None, pm_view=None, max_lines=3, compact=True, segment_char_limit=34):
    proj = str(project_name or "").strip()
    if not proj:
        return ""

    current = explanation if isinstance(explanation, dict) else build_project_current_explanation(proj, pm_view=pm_view)
    source_name = str(current.get("来源", "")).strip()
    content_text = str(current.get("内容", "")).strip() or "无数据"
    component_name = str(current.get("部件", "")).strip() or "全局进度"
    display_items = []

    def _component_rank(comp_name):
        comp_txt = str(comp_name or "").strip()
        if not comp_txt:
            return 999
        if "全局" in comp_txt:
            return 998
        std_components = globals().get("STD_COMPONENTS", ["头雕(表情)", "素体", "手型", "服装", "配件", "地台", "包装"])
        for idx, std_comp in enumerate(std_components):
            std_txt = str(std_comp).strip()
            if not std_txt:
                continue
            if comp_txt == std_txt or comp_txt.startswith(std_txt) or std_txt in comp_txt:
                return idx
        return len(std_components) + 1

    def _compact_line(line_text):
    ...
    if source_name in ["全局大盘", "项目日志"]:
        ...
        if group_rows:
            sorted_group_rows = sorted(
                group_rows,
                key=lambda row: (
                    -int(event_attention_priority(
                        content_text=str(row.get("text", "")).strip(),
                        action_type=str(row.get("action", "")).strip(),
                        source_module=source_name or "全局大盘",
                    )),
                    int(row.get("split_index", 9999) or 9999),
                    _component_rank(str(row.get("component", "")).strip()),
    ...
    if not display_items:
        display_items = [{
            "line": f"[{component_name}] {content_text}".strip(),
            "attention": int(event_attention_priority(content_text=content_text, action_type=str(current.get("动作", "")).strip(), source_module=source_name or "项目日志")),
            "order": 0,
        }]

    reminder_text = str(current.get("提醒内容", "")).strip()
    ...
    if reminder_text:
        ...
        if reminder_dt and (not exp_dt or reminder_dt >= exp_dt):
            ...
            exp_act = str(current.get("动作", "")).strip()
            if source_name == "全局大盘" and exp_act in ["追加最新动态", "改写最新动态"]:
                should_override_reminder = True
        reminder_item = {
            "line": reminder_line,
    ...
    if max_lines and len(cleaned_lines) > max_lines:
        hidden_count = len(cleaned_lines) - int(max_lines)
        cleaned_lines = cleaned_lines[:max_lines] + [f"... 另有{hidden_count}条（共{len(cleaned_lines)}条）"]
    if compact:
        return " ｜ ".join([_compact_line(x) for x in cleaned_lines if str(x).strip()])
    return "\n".join(cleaned_lines)
...
def render_compact_recognition_preview(prefix="识别预览", projects=None, component_name="", stage_name="", people=None, route_label="", flow_label="", extra_label=""):
    bits = []
    ...
    if route_label:
        bits.append(f"`时间去向` {route_label}")
    if flow_label:
        bits.append(f"`流转` {flow_label}")
    if extra_label:
...
def append_todo_completion_history(td, action_date=None):
    ...
    for proj_name in proj_list:
        ...
        logs.append(normalize_component_log_entry({
            "日期": str(action_date),
            "流转": "待办",
            "工序": stage_name,
            "事件": event_text,
        }, fallback_date=action_date))
        changed_any = True
...
def build_project_stage_segments(proj_label, proj_data):
    ...
    for stage in ["预研", "建模", "打印", "涂装", "设计", "工程", "开模", "复样", "生产"]:
        ...
        if stage in ["预研", "建模", "打印", "涂装", "设计", "工程"] and mold_start and start_dt < mold_start:
...
def render_pm_todo_manager(valid_projs, current_pm):
    ...
    role_person_options, role_person_name_map = collect_role_person_options()
    role_person_bundle = {"labels": role_person_options, "name_map": role_person_name_map}
    ...
    today = datetime.date.today()
    todo_norm_sig = todo_manager_norm_signature(todo_all, valid_projs=valid_projs)
    if st.session_state.get("todo_manager_norm_sig", "") != todo_norm_sig:
        touched = False
        for td in todo_all:
        ...
        if touched:
            persist_project_scope_batch([], recompute_projects=False)
        st.session_state["todo_manager_norm_sig"] = todo_manager_norm_signature(todo_all, valid_projs=valid_projs)

    for td in todo_all:
        scope_hint = str(td.get("所属视角", "")).strip()
    ...
    with st.container():
        ...
        queue_clicked = False
        with st.form("todo_quick_add_form", clear_on_submit=False):
            ...
            if st.session_state.get("todo_global_autofill_sig", "") != auto_sig:
                todo_auto = infer_todo_form_defaults(
                    ...
                    role_person_bundle=role_person_bundle,
                )
            ...
            people_bundle = {"labels": [], "ambiguous": [], "unknown": []}
            if people_input:
                people_bundle = safe_infer_todo_people_bundle(tmp_td, options_bundle=role_person_bundle)
            if people_input and (people_bundle["ambiguous"] or people_bundle["unknown"]):
                st.warning("人员没有唯一识别，建议在待办里直接补充【关联人员】；图片和细节再放到下方进度明细。")

            q1, q2, q3 = st.columns([1.0, 1.2, 4.0])
            with q1:
                preview_clicked = st.form_submit_button("🔎 识别预览", use_container_width=True)
            with q2:
                queue_clicked = st.form_submit_button("➕ 加入待保存列表", type="secondary", use_container_width=True)
            with q3:
                st.caption("为了提速，上方输入改成提交式；点【识别预览】或【加入待保存列表】时才会刷新。")
    ...
    load_todo_diagnostics = st.checkbox(
        "加载识别诊断列",
        value=bool(st.session_state.get("todo_load_diagnostics", False)),
        key="todo_load_diagnostics",
        help="关闭后可明显减轻输入和刷新时的整页计算；需要排查人员识别/开定识别/联动状态时再打开。",
    )
    if not load_todo_diagnostics:
        st.caption("识别诊断列已延迟加载；写待办时建议先保持关闭。")
    ...
    for td in sorted_items:
        ...
        if load_todo_diagnostics:
            row_obj["人员识别"] = format_todo_people_hint(td, options_bundle=role_person_bundle)
            row_obj["开定识别"] = infer_todo_target_hint(td, valid_projs)
    ...
    if load_todo_diagnostics:
        ...
        st.caption("字段说明：✍ 用户填写（完成/任务/CPDDL）；⚙ 系统自动（完成于/到期/提醒）。")
    ...
    if load_todo_diagnostics:
        ...
        todo_column_config["联动状态"] = st.column_config.TextColumn("联动状态 ⚙", disabled=True, width="medium")
        todo_disabled_cols.extend(["开定识别", "联动状态", "人员识别"])
    ...
    if st.button("💾 保存待办变更（含新增）", key="todo_save_global"):
        ...
        for row in edited_df.to_dict("records"):
            ...
            is_pending_create = bool(td.get("_待保存新增"))
            ...
            if changed_core and (not is_pending_create):
            ...
            if new_done and (is_pending_create or not prev_done):
                done_date = resolve_todo_completion_event_day(
                    title,
                    cpddl,
                    temporal_info=temporal,
            ...
            if is_pending_create or changed_core or (new_done != prev_done):
                todo_action = "待办更新"
                event_day = datetime.date.today()
                if is_pending_create:
                    todo_action = "待办完成" if new_done else "待办新建"
                    if new_done:
                        event_day = resolve_todo_completion_event_day(
                            title,
                            cpddl,
                            temporal_info=temporal,
                            ref_date=datetime.date.today(),
                        )
                elif new_done and not prev_done:
                    todo_action = "待办完成"
                    event_day = resolve_todo_completion_event_day(
                        title,
                        cpddl,
...
def save_fastlog_rows_to_db(edited_df, rec_date, source_name, image_bindings=None, auto_learn=True, auto_learn_chars=6,
    ...
    for _, row in edited_df.iterrows():
        ...
        append_component_log_entry(proj, comp, {
            "日期": str(event_day),
            "流转": source_name,
            "工序": final_stage,
            "事件": evt,
...
def render_pm_fastlog_integrated(sel_proj, show_header=True):
    if show_header:
        st.markdown("#### 🕰️ 补历史 / 速记补录（可指定日期）")
        st.caption("想补过去某一天的信息，就在这里先选日期再写。过去日期进历史，未来日期进待办。")
        st.caption("【数据维护】更适合回看和修改已有记录；这里才是新增入口。")
    else:
        st.caption("想补过去某一天的信息，就在这里先选日期再写。过去日期进历史，未来日期进待办。")
...
def _append_packaging_standard_event(project_name, action_type, content_text, event_date=None, extra_payload=None):
    proj = str(project_name or "").strip()
    if (not proj) or proj not in db or proj == "系统配置":
        return False
    evt_day = event_date if isinstance(event_date, datetime.date) else datetime.date.today()
    content = str(content_text or "").strip()
    if not content:
        return False
    return append_standard_event_entry(
        ...
        event_date=evt_day,
        component_name="包装",
        stage_name="设计" if any(x in content for x in ["彩盒", "灰箱", "物流箱", "地台贴", "说明书", "电影票", "感谢信"]) else "",
...
def render_packing_lightweight_board(sel_proj, ui_prefix="pack_board"):
    ...
    if st.button("➕ 添加提审轮次", key=f"{ui_prefix}_review_add"):
        if not rv_targets:
            ...
            _append_packaging_standard_event(
                sel_proj,
                "包装提审记录",
                f"包装提审：{' / '.join([x for x in rv_targets if str(x).strip()])}｜{rv_result}" + (f"｜原因：{str(rv_reason).strip()}" if rv_result == "打回" and str(rv_reason).strip() else ""),
                event_date=rv_date,
                extra_payload={"提审对象": [x for x in rv_targets if str(x).strip()], "结果": rv_result},
            )
    ...
    if st.button("💾 保存包装看板", type="primary", key=f"{ui_prefix}_save"):
        changed_labels = []
        key_label_map = dict(stage1_items + stage2_items + manual_items + stage4_items + stage5_items)
        for key, label in key_label_map.items():
            if bool(checklist.get(key, False)) != bool(new_checklist.get(key, False)):
                changed_labels.append(label + ("=完成" if bool(new_checklist.get(key, False)) else "=取消"))
        if bool(cfg.get("需要大物流箱", False)) != bool(need_large_box):
            changed_labels.append("大物流箱需求已调整")
        if str(cfg.get("重量信息", "")).strip() != str(weight_info).strip():
            changed_labels.append("重量信息已更新")
        board["配置"] = {"需要大物流箱": bool(need_large_box), "重量信息": str(weight_info).strip()}
        board["阶段清单"] = new_checklist
        board["提审记录"] = review_rounds
        db[sel_proj]["包装专项"] = board
        if changed_labels:
            _append_packaging_standard_event(
                sel_proj,
                "包装看板更新",
                "包装看板更新：" + "；".join(changed_labels[:6]),
                extra_payload={"完成节点数": sum(1 for v in new_checklist.values() if bool(v))},
            )
        sync_save_db(sel_proj)
        st.success("包装看板已保存。")
        st.rerun()
...
def _normalize_visible_print_projects(visible_projects):
    out = []
    for proj in visible_projects if isinstance(visible_projects, list) else []:
        proj_txt = str(proj or "").strip()
        if (not proj_txt) or proj_txt == "系统配置" or proj_txt not in db:
            continue
        if proj_txt in out:
            continue
        out.append(proj_txt)
...
def _split_print_tracking_rows_by_scope(stored_rows, visible_projects):
    normalized_rows = _normalize_print_tracking_rows(stored_rows)
    ...
    for row in normalized_rows:
        proj_txt = str((row or {}).get("项目", "")).strip()
        if proj_txt in scope_set:
            scoped_rows.append(dict(row))
        else:
            outside_rows.append(dict(row))
    return scoped_rows, outside_rows, scope_projects


def _merge_print_tracking_rows_for_scope(stored_rows, scoped_rows, visible_projects):
    _, outside_rows, scope_projects = _split_print_tracking_rows_by_scope(stored_rows, visible_projects)
    normalized_scope_rows = _normalize_print_tracking_rows(scoped_rows)
    if not scope_projects:
        return normalized_scope_rows
    return _normalize_print_tracking_rows(outside_rows + normalized_scope_rows)
...
def render_print_tracking_board(pm_view, visible_projects, ui_prefix="print_track"):
    ...
    cfg = db.setdefault("\u7cfb\u7edf\u914d\u7f6e", {})
    stored_rows_all = cfg.get("\u6253\u5370\u8ffd\u8e2a\u5217\u8868", []) if isinstance(cfg.get("\u6253\u5370\u8ffd\u8e2a\u5217\u8868", []), list) else []
    ...
    if project_locked_scope:
        st.caption(f"\u5f53\u524d\u662f\u9879\u76ee\u7ea7\u6253\u5370\u8ffd\u8e2a\u89c6\u56fe\uff1a\u672c\u6b21\u4fdd\u5b58\u53ea\u4f1a\u5f71\u54cd\u3010{scoped_projects[0]}\u3011\u8fd9\u4e00\u9879\uff0c\u5176\u4ed6\u9879\u76ee\u8bb0\u5f55\u4f1a\u539f\u6837\u4fdd\u7559\u3002")
    elif scoped_projects and stored_rows_outside_scope:
        st.caption(f"\u5f53\u524d\u53ea\u5c55\u793a\u6240\u9009\u8303\u56f4\u5185\u7684\u6253\u5370\u8ffd\u8e2a\uff1b\u4fdd\u5b58\u65f6\u4f1a\u4fdd\u7559\u8303\u56f4\u5916\u7684 {len(stored_rows_outside_scope)} \u6761\u8bb0\u5f55\u3002")
    ...
    with st.container(border=True):
        ...
        if st.button("\u2795 \u6dfb\u52a0\u6253\u5370\u8ffd\u8e2a", key=f"{ui_prefix}_add_btn", type="primary"):
            if not str(add_proj).strip() or add_proj not in db:
                ...
                cfg["\u6253\u5370\u8ffd\u8e2a\u5217\u8868"] = _merge_print_tracking_rows_for_scope(
                    stored_rows_all,
                    merged_rows,
                    scope_projects,
                )
                cfg["\u6253\u5370\u5730\u70b9\u9009\u9879"] = locations
                _append_print_tracking_standard_event(new_row, action_type="打印追踪新增")
                sync_save_db("\u7cfb\u7edf\u914d\u7f6e")
    ...
    edited_df = st.data_editor(
        ...
        column_config={
            ...
            "\u9879\u76ee": st.column_config.TextColumn("\u9879\u76ee", width="medium", disabled=project_locked_scope),
        ...
        disabled=["_id", "\u6765\u6e90"] + (["\u9879\u76ee"] if project_locked_scope else []),
    )
    ...
    if st.button("\U0001f4be \u4fdd\u5b58\u6253\u5370\u8ffd\u8e2a", key=f"{ui_prefix}_save"):
        ...
        for rec in edited_df.to_dict("records"):
            ...
            ) or bool(old_row.get("已收到", False)) != bool(row.get("已收到", False))
...
def render_dashboard_plan_board(valid_projects):
    ...
    if not rows:
        st.caption("当前视角下暂无计划排期数据。可先在【项目空间】里维护项目基础信息和计划排期。")
        return
...
if menu == MENU_HOME:
    ...
    def _build_dash(proj_list_key: str, db_hash: str):
        ...
        for proj in valid_projs:
            data = db[proj]
            explanation = build_project_current_explanation(proj, pm_view=current_pm)
            exp_dt = clamp_timeline_date(parse_date_safe(explanation.get("日期", "")))
            ce = build_dashboard_dynamic_display(proj, explanation=explanation, pm_view=current_pm) or (
                f"[{str(explanation.get('部件', '')).strip() or '全局进度'}] {str(explanation.get('内容', '')).strip() or '无数据'}"
            )

            if not data.get('部件列表') and not data.get('Milestone') and not data.get('Target'):
                latest_dt_for_gap = exp_dt
                dt_txt = format_gap_days_text(latest_dt_for_gap)
                _table.append({"状态":"⚪ 未知阶段","项目":proj,"跟单":"","项目当前阶段":"待立项",
                    "开定时间":"TBD","预计发货":"-","断更":dt_txt,"最新全盘动态":ce}); continue
            ...
            if not comps:
                ...
                _table.append({"状态":r_txt,"项目":proj,"跟单":gd,"项目当前阶段":ms,
                    "开定时间":tgt,"预计发货":ship_itv,"断更":dt_txt,"最新全盘动态":ce}); continue
            ...
            dt_txt = format_gap_days_text(latest_dt_for_gap)
            ce = build_dashboard_dynamic_display(proj, explanation=explanation, pm_view=current_pm) or (
                f"[{str(explanation.get('部件', '')).strip() or latest_comp_name or '全局进度'}] {str(explanation.get('内容', '')).strip() or latest_event_str}"
            )
            _table.append({"状态":r_txt,"项目":proj,"跟单":gd,"项目当前阶段":ms,
                "开定时间":tgt,"预计发货":ship_itv,"断更":dt_txt,"最新全盘动态":ce})
            _meta.append({
                "项目": proj,
                "项目标签": proj_y_label,
    ...
    if gantt_data:
        ...
        st.markdown("#### ⏱️ 预研/建模/打印/涂装/设计/工程平均耗时（天）")
        ...
        focus = df_dur[df_dur["工序阶段"].isin(["预研", "建模", "打印", "涂装", "设计", "工程"])]
        rows = []
        for stg in ["预研", "建模", "打印", "涂装", "设计", "工程"]:
    ...
    if table_data:
        ...
        def _apply_latest_dynamic_update(project_name, new_text, mode, auto_save=True, event_date=None, raw_text_for_target="", segment_rows=None):
            proj = str(project_name or "").strip()
            ...
            appended_any = False
            normalized_rows = [row for row in (segment_rows or []) if isinstance(row, dict)]
            if normalized_rows:
                batch_write_time = datetime.datetime.now().isoformat(timespec="seconds")
                split_total = len(normalized_rows)
                first_comp = ""
                for row_idx, row in enumerate(normalized_rows, start=1):
                    row_text = str(row.get("text", "")).strip() or msg
                    row_comp = str(row.get("component", "")).strip() or target_comp
                    row_stage = str(row.get("stage", "")).strip() or curr_stage
                    if row_comp == "🌐 全局进度 (Overall)":
                        row_comp = "全局进度"
                    if row_comp not in comps or not isinstance(comps.get(row_comp), dict):
                        comps[row_comp] = {"主流程": STAGES_UNIFIED[0], "日志流": []}
            ...
            for row in edited_dashboard_df.to_dict("records"):
                ...
                if latest_raw_after != latest_raw_before:
                    ...
                    if not latest_after:
                        ...
                        history_segment_rows = None
                        if row_dynamic_mode == "append_latest":
                            history_target_comp = _infer_component_for_dynamic(proj, latest_raw_after)
                            history_target_stage = str(
                                db.get(proj, {}).get("部件列表", {}).get(history_target_comp, {}).get("主流程", "")
                            ).strip()
                            history_segment_rows = expand_dashboard_dynamic_history_entries(
                                latest_after,
                                project_name=proj,
                                ref_date=event_date or datetime.date.today(),
                                fallback_component=history_target_comp,
                                fallback_stage=history_target_stage,
                            )
                        ...
                        ok, msg = _apply_latest_dynamic_update(
                            ...
                            segment_rows=history_segment_rows,
                        )
                        if ok:
...
# ==========================================
    ...
    with st.container():
        ...
        if not valid_projs:
            st.warning("当前视角下暂无项目；可以先在上方手动建档，或先去【我的待办】整理提醒。")
            st.stop()
        ...
        proj_explain = build_project_current_explanation(sel_proj, pm_view=current_pm)
        sel_proj_data = db.get(sel_proj, {})
        sel_proj_pending_todos = [
            x for x in todo_list
            if (not x.get("完成")) and todo_matches_project(x, sel_proj)
        ]

        with st.container(border=True):
            st.markdown(f"**📍 项目操作上下文：{sel_proj}**")
            project_head_bits = [
                f"负责人：{str(sel_proj_data.get('负责人', '')).strip() or '-'}",
                f"项目阶段：{str(sel_proj_data.get('Milestone', '')).strip() or '-'}",
                f"开定：{str(sel_proj_data.get('Target', '')).strip() or 'TBD'}",
                f"发货：{str(sel_proj_data.get('发货区间', '')).strip() or '-'}",
        ...
        if project_detail_view == "📌 概览":
            ...
            if not comps:
                ...
                for matrix_row in matrix_rows:
                    ...
                    guantu_complete_stages = {"建模(含打印/签样)", "设计", "工程拆件", "手板/结构板"}
            ...
            with st.container(border=True):
                ...
                with qa1:
                    if st.button("🕰️ 去数据维护看项目历史", key=f"proj_jump_hist_{norm_text(sel_proj)}", use_container_width=True):
                        open_project_maintenance(sel_proj)
                with qa2:
                    if st.button("🔗 去数据维护看统一事件", key=f"proj_jump_evt_{norm_text(sel_proj)}", use_container_width=True):
                        open_project_maintenance(sel_proj, load_standard_events=True)
                with qa3:
        ...
        if project_detail_view == "📝 进展":
            ...
            with st.expander("📋 最近文件流转 / 日志时间线（可挂图）", expanded=False):
            ...
            with st.container(border=True):
                ...
                current_comp_choice = st.session_state.get(comp_choice_key, wb_comp_opts[0] if wb_comp_opts else "🌐 全局进度 (Overall)")
                actual_stage_seed = stage_default_name
                if current_comp_choice == "🌐 全局进度 (Overall)":
                    actual_stage_seed = infer_default_handoff_stage(sel_proj, "全局进度")
                elif current_comp_choice and current_comp_choice not in ["➕ 新增细分配件..."]:
                    actual_stage_seed = infer_default_handoff_stage(sel_proj, current_comp_choice)
                if actual_stage_seed not in STAGES_UNIFIED:
                    actual_stage_seed = stage_default_name if stage_default_name in STAGES_UNIFIED else (STAGES_UNIFIED[0] if STAGES_UNIFIED else "立项")
                stage_default_idx = STAGES_UNIFIED.index(actual_stage_seed) if actual_stage_seed in STAGES_UNIFIED else 0

                a1, a2, a3, a4 = st.columns([1.3, 1.1, 1.2, 1.2])
                with a1:
                    wb_comp_raw = st.selectbox("操作部件", wb_comp_opts, key=f"wb_comp_{fk}")
                with a2:
                    wb_stage = st.selectbox("目标阶段", STAGES_UNIFIED, index=stage_default_idx, key=f"wb_stage_{fk}")
                with a3:
                    wb_evt_type = st.selectbox("记录类型", FILE_FLOW_RECORD_TYPES, key=f"wb_evt_{fk}")
                with a4:
                ...
                if st.button("💾 保存交接记录", type="primary", key=f"wb_save_{sel_proj}_{fk}"):
                    if wb_comp_raw == "➕ 新增细分配件..." and not wb_new_comp_name:
                        ...
                        if warning_lines and not force_submit_detail:
                            ...
                            if temporal_route == "todo" and isinstance(temporal_date, datetime.date):
                                todo_result = upsert_todo_from_event_text(
                                    sel_proj,
                                    str(wb_text).strip(),
                                    forced_due_dt=temporal_date,
                                    forced_task_body=temporal_body,
                                ...
                                todo_state, auto_todo_ids = unpack_todo_upsert_result(todo_result)
                                linked_event_todo_ids = list(dict.fromkeys([x for x in (linked_todo_ids + auto_todo_ids) if x]))
                                append_standard_event_entry(
                                    source_module="PM工作台",
                                    action_type="工作台联动待办",
                                    ...
                                    todo_ids=linked_event_todo_ids,
                                    actor=current_pm if current_pm != "所有人" else "系统",
                                    intent="todo",
                            ...
                            if linked_todo_ids:
                                ...
                                link_module = todo_link_module_label("PM工作台", "工作台记录")
...
* ✅ **多管线并发**：`1/6萨鲁曼：头雕打样中；目标定价2980；法杖需要修改` -> 自动拆分为 3 笔独立记录
    ...
    if st.session_state.parsed_logs:
        ...
        st.markdown("**🖼️ 附件图片**")
        st.caption("附件会跟随每条入库记录自动关联到其对应项目/部件，后续可在【数据维护】按项目追溯。")
        ...
        if st.button("💾 确认入库", type="primary"):
            ...
            for log in edited_logs:
                ...
                append_component_log_entry(p, target_comp, {
                    "日期": td, "流转": "AI速记",
...
# ==========================================
    ...
    st.caption("项目备忘录请在【项目空间】维护；入库/领用台账请在成本台账查看。")
...
# ==========================================
    ...
    if flat_data and maintenance_show_cross:
        df_logs = pd.DataFrame(flat_data).sort_values(by="日期", ascending=False).reset_index(drop=True)
        df_logs.insert(0, '序号', range(len(df_logs), 0, -1))
        ...
        with st.expander("📅 按日查看 / 修正操作历史", expanded=True):
            ...
            if day_scope_mode == "当前项目":
                ...
                st.caption("这里和当前项目上下文保持一致；如果上面还筛了特定部件，这里也会一起跟随。")
    ...
    elif maintenance_show_history and flat_data:
        render_project_history_editor_block(
            sel_proj,
            sel_comp,
            current_pm,
    ...
    elif maintenance_show_history or maintenance_show_cross:
        st.info("该过滤条件下暂无记录。")
...
# ==========================================
    ...
    st.info("👋 新同事建议按下面顺序操作：先看今日视图，再进项目空间录入，最后用数据维护校验历史。")
    st.markdown("---")
    ...
    with st.expander("📁 项目空间：项目推进 + 透视矩阵", expanded=True):
        st.markdown(
            ...
            "3. **包装快捷跟踪**：可在项目空间直接勾选包装状态；附件追溯在包装模块维护。"
        )

    with st.expander("📝 AI 速记：最低成本自动学习", expanded=False):
        st.markdown(
            "1. **进入项目空间**：点击左侧 **【📁 项目空间】**，先选项目再补项目更新。\n"
    ...
    with st.expander("🧯 系统设置与风险提示", expanded=False):
        st.markdown(
            "1. 合并同类项目支持历史恢复与多选删除。\n"
