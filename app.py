...
# ==========================================
# 🚀 核心架构：本地化无感画质压缩引擎 (解决 JSON 臃肿)
# ==========================================
IMG_DIR = "img_assets"
if not os.path.exists(IMG_DIR): os.makedirs(IMG_DIR)

def compress_to_b64(img_data, max_size=(1000, 1000), quality=70):
    try:
...
STD_COSTS_LIST = ["研发费", "模具费", "大货生产", "包装印刷", "物流运输", "外包设计", "杂项其他"] # 修复 Bug: 定义默认成本分类
...
class DatabaseManager:
    def __init__(self, file_path="tracker_data_web_v20.json"): self.file_path = file_path
    def load(self):
...
def init_session():
    ...
    if "系统配置" not in st.session_state.db: st.session_state.db["系统配置"] = DEFAULT_SYS_CFG
    for k, v in DEFAULT_SYS_CFG.items():
        if k not in st.session_state.db["系统配置"]: st.session_state.db["系统配置"][k] = v
...
def auto_sync_milestone(proj_name):
    ...
    if not proj_data or "部件列表" not in proj_data: return
    comps = proj_data['部件列表']
    max_idx = -1; max_stage = None
    for c_name, info in comps.items():
        ...
        if s_idx > max_idx and "暂停" not in STAGES_UNIFIED[s_idx]:
            max_idx = s_idx; max_stage = STAGES_UNIFIED[s_idx]

    if max_idx >= 0 and max_stage:
        global_key = next((k for k in comps.keys() if "全局" in k), "全局进度")
        if global_key not in comps: comps[global_key] = {"主流程": STAGES_UNIFIED[0], "日志流": []}
        ...
        if curr_idx < max_idx and "暂停" not in curr_global_stage:
            for fill_idx in range(max(0, curr_idx + 1), max_idx + 1):
                ...
                if "暂停" in fill_stage: continue
                evt_txt = f"[系统自动追踪] 因子部件到达【{max_stage}】，全局被倒逼流转" if fill_idx != max_idx else f"[系统自动追踪] 因子部件到达【{max_stage}】，全局对齐！"
                comps[global_key].setdefault("日志流", []).append({"日期": str(datetime.date.today()), "流转": "系统自动", "工序": fill_stage, "事件": evt_txt})
    ...
    if all(s == "✅ 已完成(结束)" for s in stages) and stages: proj_data['Milestone'] = "项目结束撒花🎉"
...
def sync_save_db():
    for p in st.session_state.db: 
        if p != "系统配置": auto_sync_milestone(p)
    latest_db = db_manager.load()
    for k, v in st.session_state.db.items(): latest_db[k] = v
...
def get_risk_status(milestone, target_date_str="TBD"):
    ms, target_date_str = str(milestone).strip(), str(target_date_str).strip()
...
valid_projs = [p for p, d in db.items() if p != "系统配置" and (current_pm == "所有人" or str(d.get('负责人', '')).strip() == current_pm)]

# 🚨 修复 NameError 的核心关键：必须在此定义 menu 变量！
menu = st.sidebar.radio("模块导航", [MENU_DASHBOARD, MENU_SPECIFIC, MENU_FASTLOG, MENU_PACKING, MENU_COST, MENU_HISTORY, MENU_SETTINGS, MENU_GUIDE])

# 🚀 ZIP 全量打包备份与恢复
st.sidebar.divider()
st.sidebar.markdown("### ⚙️ 数据备份与恢复 (全量包)")
try:
    ...
    zip_buffer.seek(0)
    st.sidebar.download_button("💾 下载全量备份 (数据+图片)", data=zip_buffer, file_name=f"inart_pm_full_backup_{datetime.date.today()}.zip", mime="application/zip")
except Exception as e: st.sidebar.warning(f"备份生成失败: {e}")
        ...
        if restore_file.name.endswith(".zip"):
            with zipfile.ZipFile(restore_file, "r") as zf:
                if "database.json" in zf.namelist():
                    with zf.open("database.json") as f: restored_data = json.load(f)
                else:
                    st.sidebar.error("❌ 压缩包内未找到 database.json！"); st.stop()

                if not os.path.exists("img_assets"): os.makedirs("img_assets")
                for item in zf.namelist():
                    if item.startswith("img_assets/") and not item.endswith('/'):
                        zf.extract(item, path=".") 
        else: restored_data = json.load(restore_file)

        db_manager.save(restored_data); st.session_state.db = restored_data
        st.sidebar.success("🎉 恢复成功！图片资产及数据已同步还原，请手动刷新网页！")
    except Exception as e: st.sidebar.error(f"解析失败: {e}")
...
if menu == MENU_DASHBOARD:
    ...
    with st.expander("📥 批量导入/更新研发总表 (CSV)"):
        st.info("💡 提示：支持自动识别含有【项目名称】、【负责人】、【当前阶段】、【开定时间】、【发货区间】、【跟单】等列的 CSV 文件。如果项目已存在，将自动更新其大盘信息。")
        rd_csv = st.file_uploader("选择研发总表 CSV 文件", type=['csv'], key="rd_csv_uploader")
        if rd_csv and st.button("🚀 开始解析导入", type="primary"):
            try:
                try: df_rd = pd.read_csv(rd_csv)
                except UnicodeDecodeError: 
                    rd_csv.seek(0); df_rd = pd.read_csv(rd_csv, encoding='gbk')
                ...
                if not col_proj:
                    st.error("❌ 未能在CSV中找到代表【项目名称】的列，请检查表头！")
                else:
                    ...
                    for _, row in df_rd.iterrows():
                        ...
                        if p_name not in db:
                            db[p_name] = {"负责人": pm_val, "跟单": gd_val, "Milestone": ms_val, "Target": tgt_val, "发货区间": ship_val, "部件列表": {}, "发货数据": {}, "成本数据": {}}
                            count_new += 1
                        else:
                    ...
                    sync_save_db(); st.success(f"🎉 导入完毕！新增项目: {count_new} 个，更新项目: {count_update} 个。"); st.rerun()
    ...
    for proj in valid_projs:
        data = db[proj]
        gd = data.get('跟单', ''); ms = data.get('Milestone', ''); tgt = data.get('Target', 'TBD'); ship_itv = data.get('发货区间', '-')
        ...
        if not comps:
            table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "项目当前阶段": ms, "开定时间": tgt, "预计发货": ship_itv, "断更": "-", "最新全盘动态": "无数据"})
        else:
            ...
            for c_name, info in comps.items():
                ...
                for pair in re.split(r'[,，|]', owner_str):
                    ...
                    if '-' in pair:
                        ...
                        r_part, p_part = pair.split(':', 1)
                        project_person_roles.add((proj, p_part.strip(), r_part.strip()))
                    else: project_person_roles.add((proj, pair.strip(), "综合"))
                ...
                for log in logs:
                    ...
                    try: 
                        ...
                        if key not in grouped_logs_dict:
                            grouped_logs_dict[key] = {"日期_obj": dt_obj, "日期_str": log['日期'], "工序": macro_stage, "事件": evt_txt, "部件": [c_name]}
                        else:
            ...
            if "补充:" in clean_event: clean_event = clean_event.split("补充:")[-1].split("[系统]")[0].strip()
            elif "】" in clean_event: clean_event = clean_event.split("】")[-1].split("[系统]")[0].strip()

            table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "项目当前阶段": ms, "开定时间": tgt, "预计发货": ship_itv, "断更": dt_txt, "最新全盘动态": f"[{latest_comp_name}] {clean_event}"})

            parsed_tgt = None
            try:
                if tgt and tgt.upper() != 'TBD':
                    parsed_tgt = datetime.datetime.strptime(f"{tgt}-01" if len(tgt) == 7 else tgt[:10], "%Y-%m-%d")
            ...
            if all_logs:
                ...
                for i, log in enumerate(all_logs):
                    ...
                    if is_last or nxt_stage != curr_stage:
                        ...
                        gantt_data.append({"项目": proj_y_label, "工序阶段": curr_stage, "Start": s_dt.strftime("%Y-%m-%d"), "Finish": e_dt.strftime("%Y-%m-%d"), "详情": "<br>".join([f"• {e}" for e in cache])})
                        if not is_last: curr_stage = nxt_stage; s_dt = log["日期_obj"]; cache = []

    st.divider(); st.subheader("📈 全局进展甘特图")
    if gantt_data:
        df_g = pd.DataFrame(gantt_data).sort_values(by=["项目", "Start"])
        fig = px.timeline(df_g, x_start="Start", x_end="Finish", y="项目", color="工序阶段", hover_name="详情", category_orders={"工序阶段": gantt_cat_orders}, color_discrete_map=combined_color_map)
        if star_x: 
            fig.add_trace(go.Scatter(x=star_x, y=star_y, mode='markers', marker=dict(symbol='star', size=24, color='#FFD700', line=dict(width=2, color='#FF4500')), name='📅 目标开定', hovertemplate='目标开定: %{x}<extra></extra>'))
    ...
    if project_person_roles:
        ...
        fig_owner = px.bar(df_owner, x='人员', y='积压项目数', color='职务', title="👤 团队&责任人 Loading (按参与项目数统计)", text='积压项目数')
...
# ==========================================
            ...
            with c_n3: 
                ...
                if st.button("✅ 确认创建", type="primary"):
                    if new_p and new_p not in db:
                        db[new_p] = {"负责人": new_pm, "跟单": "", "Milestone": "待立项", "Target": "TBD", "发货区间": "", "部件列表": {}, "发货数据": {}, "成本数据": {}}
                        sync_save_db(); st.success(f"建档成功！已分配给 {new_pm}"); st.session_state.new_proj_mode = False; st.rerun()

    if not valid_projs: st.warning(f"当前视角下暂无项目。"); st.stop()

    if 'current_proj_context' not in st.session_state: st.session_state.current_proj_context = valid_projs[0] if valid_projs else None
    sel_proj = st.selectbox("📌 1. 选择要透视与操作的项目 (💡支持键盘打字模糊搜索)", valid_projs)
    if sel_proj != st.session_state.current_proj_context:
        st.session_state.pasted_cache = {}; st.session_state.config_pasted_cache = {}
        st.session_state.exclude_imgs = set(); st.session_state.config_consumed_hashes = set()
        st.session_state.current_proj_context = sel_proj

    st.divider(); st.subheader("🔬 项目进度透视矩阵 (并行连消追踪)")
    ...
    if not comps: st.warning("暂无录入部件明细。请在下方录入。")
        ...
        for comp_name in y_labels:
            ...
            if active_stages or completed_stages: active_stages.discard("立项"); completed_stages.add("立项")
            ...
            for i in range(len(STAGES_UNIFIED)):
                ...
                elif (real_c_idx >= guan_tu_idx and i < real_c_idx and "暂停" not in stg) or (stg in completed_stages) or (cur_stage == "✅ 已完成(结束)"):
        ...
        fig_grid = go.Figure(data=go.Heatmap(z=z_data, x=STAGES_UNIFIED, y=y_labels_display, colorscale=colorscale, showscale=False, xgap=4, ygap=4, text=hover_text, hoverinfo='text'))
        fig_grid.update_layout(xaxis=dict(side='top', tickangle=-45), yaxis=dict(autorange='reversed', automargin=True), plot_bgcolor='white', height=max(250, len(y_labels) * 45), margin=dict(t=120, b=20, r=20))
        st.plotly_chart(fig_grid, use_container_width=True)

    st.divider(); st.subheader("🔧 进度明细与流转交接工作台")
    cur_pm = db[sel_proj].get('负责人', 'Mo'); cur_ms = db[sel_proj].get('Milestone', '')
    cur_target = db[sel_proj].get('Target', 'TBD'); cur_ship = db[sel_proj].get('发货区间', '') 
    ...
    with col_m4: new_ship = st.text_input("📦 预计发货区间 (例: 2026 Q2)", value=cur_ship)

    if st.button("💾 更新大盘基础信息", type="primary", key="btn_global"): 
        db[sel_proj]['负责人'] = new_pm; db[sel_proj]['Milestone'] = new_ms; db[sel_proj]['Target'] = new_target; db[sel_proj]['发货区间'] = new_ship
        td = str(datetime.date.today()); comps_list = list(db[sel_proj].get('部件列表', {}).keys())
        ...
        if t_c not in db[sel_proj].setdefault("部件列表", {}): db[sel_proj]["部件列表"][t_c] = {"主流程": STAGES_UNIFIED[0], "日志流": []}
        db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": db[sel_proj]["部件列表"][t_c]["主流程"], "事件": f"[属性更新] 阶段:{new_ms} | 开定:{new_target} | 发货:{new_ship}"})
    ...
    with st.expander("📄 产品配置清单 (图文长图底稿)"):
        ...
        if new_link != curr_link: db[sel_proj]["配件清单链接"] = new_link; sync_save_db(); st.rerun()
        if curr_link: st.markdown(f"👉 **[点击此处，在新标签页打开产品配置清单]({curr_link})**")
        st.divider()
        st.markdown("**🖼️ 上传清单截图 (按 Ctrl+V 直接粘贴)**")

        try:
            from streamlit_paste_button import paste_image_button
            config_paste_result = paste_image_button("📋 专属底稿截图捕获区", background_color="#f8f9fa", hover_background_color="#e2e8f0", key=f"paste_cfg_{sel_proj}")
            if config_paste_result is not None and hasattr(config_paste_result, 'image_data') and config_paste_result.image_data is not None:
                buffered = io.BytesIO()
                config_paste_result.image_data.save(buffered, format="PNG")
                h_key = hashlib.md5(buffered.getvalue()).hexdigest()
                if h_key not in st.session_state.config_pasted_cache and h_key not in st.session_state.config_consumed_hashes:
                    st.session_state.config_pasted_cache[h_key] = config_paste_result.image_data
        except ImportError: pass

        config_files = st.file_uploader("或直接拖拽长图/截图", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True, key=f"cfg_up_{sel_proj}")
        if st.session_state.config_pasted_cache:
            ...
            for i, (k, img) in enumerate(st.session_state.config_pasted_cache.items()):
                with cfg_p_cols[i % 4]:
                    ...
                    if st.button("🗑️ 移除", key=f"del_cfg_paste_{k}", use_container_width=True): keys_to_del.append(k)
            if keys_to_del:
                for k in keys_to_del: del st.session_state.config_pasted_cache[k]; st.session_state.config_consumed_hashes.add(k)
                st.rerun()
        ...
        if config_files or st.session_state.config_pasted_cache:
            if st.button("💾 保存图片为底稿", type="secondary"):
                b64_drafts = []
                if config_files:
                    for f in config_files: b64_drafts.append(compress_to_b64(f.read()))
                for k, img_obj in st.session_state.config_pasted_cache.items():
                    b64_drafts.append(compress_to_b64(img_obj)); st.session_state.config_consumed_hashes.add(k)
                db[sel_proj]["配件清单长图"] = db[sel_proj].get("配件清单长图", []) + b64_drafts
                st.session_state.config_pasted_cache = {}; sync_save_db(); st.success("✅ 保存成功！"); st.rerun()
            ...
            for idx, b64_str in enumerate(saved_drafts):
                with draft_cols[idx % 2]:
                    ...
                    if st.button("🗑️ 移除此底稿", key=f"del_draft_{sel_proj}_{idx}"):
                        saved_drafts.pop(idx); db[sel_proj]["配件清单长图"] = saved_drafts; sync_save_db(); st.rerun()
    ...
    with st.container(border=True):
        ...
        role_vals = {}; r_cols = st.columns(6)
        for idx, r in enumerate(role_list):
            with r_cols[idx]:
                old_v = old_dict.get(r, "")
                temp_opts = base_options.copy()
                if old_v and old_v not in temp_opts and old_v != "(留空/暂不分配)": temp_opts.insert(1, old_v)
                sel_val = st.selectbox(f"{r}", temp_opts, index=temp_opts.index(old_v) if old_v in temp_opts else 0, key=f"role_{r}_{fk}")
                if sel_val == "➕ 手动输入新成员...": final_val = st.text_input("👉 新姓名", key=f"rnew_{r}_{fk}")
                elif sel_val == "(留空/暂不分配)": final_val = ""
                else: final_val = sel_val
        ...
        try:
            from streamlit_paste_button import paste_image_button
            paste_result = paste_image_button("📋 剪贴板捕获区", background_color="#f1f5f9", hover_background_color="#e2e8f0", key=f"paste_log_{sel_proj}_{fk}")
            if paste_result is not None and hasattr(paste_result, 'image_data') and paste_result.image_data is not None:
                ...
                h_key = hashlib.md5(buffered.getvalue()).hexdigest()
                if h_key not in st.session_state.pasted_cache: st.session_state.pasted_cache[h_key] = paste_result.image_data
        except ImportError: pass

        img_files = st.file_uploader("或选择文件上传", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True, key=f"up_log_{sel_proj}_{fk}")
        ...
        if preview_imgs:
            ...
            for idx, img_info in enumerate(preview_imgs):
                with p_cols[idx % 6]:
                    ...
                    else: img_info["data"].seek(0); st.image(img_info["data"], use_container_width=True)
                    if st.button("🗑️ 移除", key=f"del_{img_info['id']}_{idx}", use_container_width=True, type="primary"):
                        st.session_state.exclude_imgs.add(img_info["id"]); st.rerun()

        st.markdown("---")
        is_completed = st.checkbox(f"✅ 标记所选部件的【{new_stage}】阶段已彻底完成 (矩阵变绿)", value=False, key=f"comp_{fk}")

        if st.button("🚀 批量保存交接与进度", type="primary", use_container_width=True):
            if "➕ 新增细分配件..." in comps_to_process and not new_comp_name: st.error("❌ 新增名称为空！")
            else:
                new_owner_final = ", ".join([f"{k}-{v}" for k, v in role_vals.items() if v])
                img_b64_list = []
                for img_info in preview_imgs:
                    if img_info["type"] == "paste": img_b64_list.append(compress_to_b64(img_info["data"]))
                    else: img_info["data"].seek(0); img_b64_list.append(compress_to_b64(img_info["data"].read()))
                ...
                for c_raw in comps_to_process:
                    ...
                    base_log = f"【{evt_type} | {handoff}】补充: {log_txt}" if log_txt else f"【{evt_type} | {handoff}】"
                    if is_completed: base_log += " [系统]彻底完成"

                    if new_stage == "立项":
                        db[sel_proj]["部件列表"][actual_c]['日志流'].append({"日期": str(detail_record_date), "流转": evt_type, "工序": "立项", "事件": base_log, "图片": img_b64_list})
                        db[sel_proj]["部件列表"][actual_c]['日志流'].append({"日期": str(detail_record_date + datetime.timedelta(days=1)), "流转": "系统自动", "工序": "建模(含打印/签样)", "事件": "[系统] 立项完成自动推演"})
                        db[sel_proj]["部件列表"][actual_c]['主流程'] = "建模(含打印/签样)"
                    else:
                        db[sel_proj]["部件列表"][actual_c]['日志流'].append({"日期": str(detail_record_date), "流转": evt_type, "工序": new_stage, "事件": base_log, "图片": img_b64_list})
                        db[sel_proj]["部件列表"][actual_c]['主流程'] = new_stage

                st.session_state.form_key += 1; st.session_state.pasted_cache = {}; st.session_state.exclude_imgs = set()
                sync_save_db(); st.success(f"🎉 记录成功！"); st.rerun()
    ...
    if efficiency_data: st.dataframe(pd.DataFrame(efficiency_data), use_container_width=True)
    else: st.info("💡 暂无完整闭环的工时记录。在上方交接推进时，勾选【彻底完成】，即可激活此工时排行榜！")
...
# ==========================================
    ...
    COMP_KW = {"头": "头雕(表情)", "眼": "头雕(表情)", "脸": "头雕(表情)", "手": "手型", "衣": "服装", "包": "包装", "盒": "包装", "地台": "地台", "扣": "配件", "法杖": "配件", "杯": "配件", "剑": "配件"}
    STAGE_KW = {"定价": "立项", "评估": "立项", "打印": "建模(含打印/签样)", "模型": "建模(含打印/签样)", "缩放": "建模(含打印/签样)", "修": "建模(含打印/签样)", "建模": "建模(含打印/签样)", "涂": "涂装", "色": "涂装", "设计": "设计", "原画": "设计", "拆件": "工程拆件", "官图": "官图", "大货": "大货", "完成": "✅ 已完成(结束)", "结束": "✅ 已完成(结束)"}
    ...
    if st.button("✨ 智能拆解", type="primary"):
        if not raw_text.strip(): st.warning("内容为空！")
        else:
            parsed = []; prefix_pattern = re.compile(r'^(1/6|1/4|1/12|1/3|1/1)')
            for line in [l.strip() for l in raw_text.split('\n') if l.strip()]:
                line = line.replace('：', ':') 
                if ':' not in line: continue
                proj_part, updates_part = line.split(':', 1)

                raw_projs = [p.strip() for p in re.split(r'[&和、]', proj_part) if p.strip()]
                resolved_projs = []; current_prefix = ""
                for rp in raw_projs:
                    match = prefix_pattern.search(rp)
                    if match: current_prefix = match.group(1)
                    elif current_prefix and not prefix_pattern.search(rp): rp = f"{current_prefix} {rp}".replace("  ", " ")
                    matched_p = next((vp for vp in valid_projs if rp.replace(" ", "").lower() in vp.replace(" ", "").lower()), "未知/请手动修改")
                    if matched_p == "未知/请手动修改":
                        core_raw = re.sub(r'^(1/6|1/4|1/12|1/3|1/1)\s*', '', rp).strip()
                        matched_p = next((vp for vp in valid_projs if core_raw and core_raw in vp), "未知/请手动修改")
                    resolved_projs.append(matched_p)

                for up in [u.strip() for u in re.split(r'[;；]', updates_part) if u.strip()]:
                    detected_comp = next((comp for kw, comp in DYNAMIC_COMP_KW.items() if kw in up), "全局进度")
                    detected_stage = next((stg for kw, stg in DYNAMIC_STAGE_KW.items() if kw in up), "(维持原阶段)")
                    for rp in resolved_projs: parsed.append({"识别项目": rp, "推测部件": detected_comp, "推测阶段": detected_stage, "待写入事件": up})
            st.session_state.parsed_logs = parsed; st.success("🎉 拆解完成！")
    ...
    if st.session_state.parsed_logs:
        ...
        for i, item in enumerate(st.session_state.parsed_logs):
            ...
            with c1: sel_proj = st.selectbox("归属项目", project_options, index=project_options.index(item['识别项目']) if item['识别项目'] in project_options else 0, key=f"sel_p_{i}")
            with c2: sel_comp = st.selectbox("归属部件", comp_options, index=comp_options.index(item['推测部件']) if item['推测部件'] in comp_options else 0, key=f"sel_c_{i}")
            with c3:
                options_stages = ["(维持原阶段)"] + STAGES_UNIFIED
                sel_stage = st.selectbox("AI预测", options_stages, index=options_stages.index(item['推测阶段']) if item.get('推测阶段') in options_stages else 0, key=f"stg_{i}")
            with c4: sel_event = st.text_input("📝 写入事件", value=item['待写入事件'], key=f"evt_{i}")
            with c5: ai_kw = st.text_input("🧠 提取触发新词", placeholder="如: 法杖", key=f"kw_{i}")
            edited_logs.append({"项目": sel_proj, "部件": sel_comp, "事件": sel_event, "推测阶段": sel_stage, "新词汇": ai_kw})

        st.markdown("**🖼️ 附件图片**")
        try:
            from streamlit_paste_button import paste_image_button
            ai_paste_result = paste_image_button("📋 专属剪贴板捕获区", background_color="#f1f5f9", hover_background_color="#e2e8f0", key="ai_paste_btn")
            if ai_paste_result is not None and hasattr(ai_paste_result, 'image_data') and ai_paste_result.image_data is not None:
                buffered = io.BytesIO()
        ...
        ai_files = st.file_uploader("或直接拖拽图片", type=['png', 'jpg', 'jpeg'], accept_multiple_files=True, key="ai_up_files")
        ...
        if st.session_state.ai_pasted_cache:
            ...
            for i, (k, img) in enumerate(st.session_state.ai_pasted_cache.items()):
                with cfg_p_cols[i % 6]:
                    st.image(img, use_container_width=True)
                    if st.button("🗑️ 移除", key=f"del_ai_{k}", use_container_width=True): keys_to_del.append(k)
            if keys_to_del:
                for k in keys_to_del: del st.session_state.ai_pasted_cache[k]; st.session_state.ai_consumed_hashes.add(k)
                st.rerun()
        ...
        if st.button("💾 确认入库", type="primary"):
            ...
            ai_b64_list = []
            if ai_files:
                for f in ai_files: ai_b64_list.append(compress_to_b64(f.read()))
            for k, img_obj in st.session_state.ai_pasted_cache.items():
                ai_b64_list.append(compress_to_b64(img_obj)); st.session_state.ai_consumed_hashes.add(k)

            learned_count = 0
            for log in edited_logs:
                p = log['项目']
                if p not in db or "未知" in p or "冲突" in p: st.error(f"跳过无效项目: {p}"); continue
                target_comp = log["部件"] if log["部件"] != "其他配件(系统自动创建)" else "自定义配件"

                snippet = log.get("新词汇", "").strip()
                if snippet:
                    if target_comp != "全局进度": SYS_CFG.setdefault("AI_COMP_KW", {})[snippet] = target_comp; learned_count += 1
                    if log["推测阶段"] != "(维持原阶段)": SYS_CFG.setdefault("AI_STAGE_KW", {})[snippet] = log["推测阶段"]; learned_count += 1

                if target_comp not in db[p].setdefault("部件列表", {}): db[p]["部件列表"][target_comp] = {"主流程": STAGES_UNIFIED[0], "日志流": []}
                final_stage = db[p]["部件列表"][target_comp].get("主流程", STAGES_UNIFIED[0]) if log["推测阶段"] == "(维持原阶段)" else log["推测阶段"]

                db[p]["部件列表"][target_comp]['日志流'].append({"日期": td, "流转": "AI速记", "工序": final_stage, "事件": log['事件'], "图片": ai_b64_list})
                db[p]["部件列表"][target_comp]["主流程"] = final_stage 

            sync_save_db(); st.session_state.parsed_logs = []; st.session_state.ai_pasted_cache = {}
            msg = "🎉 入库成功！" if learned_count == 0 else f"🎉 入库成功！AI 已默默学会了 {learned_count} 个新词汇！"
...
# ==========================================
    ...
    st.divider(); st.markdown("### 📝 项目全局备忘录")
    memo_txt = st.text_area("记录跨部门叮嘱等杂项", value=db[sel_proj].get("备忘录", ""), height=100)
    if st.button("💾 保存备忘录"): db[sel_proj]["备忘录"] = memo_txt; sync_save_db(); st.success("已保存！"); st.rerun()
    ...
    st.divider(); st.markdown("### 🧮 工厂大货入库与特殊领用台账")
    ...
    with c3: 
        st.write(""); 
        if st.button("💾 保存基础单量"): db[sel_proj].setdefault("成本数据", {})["总订单数"] = orders; db[sel_proj]["成本数据"]["销售单价"] = price; sync_save_db(); st.success("已保存"); st.rerun()
    ...
    st.divider(); st.subheader("➕ 手动录入单笔成本")
    ...
    if st.button("入账"):
        db[sel_proj].setdefault("成本数据", {}).setdefault("动态明细", []).append({
            ...
            "税后总成本": float(Decimal(str(c_unit)) * Decimal(str(c_qty))), "税点": f"{tax_rate}%", "税前总成本": float(round((Decimal(str(c_unit)) * Decimal(str(c_qty))) / (Decimal("1") + (Decimal(str(tax_rate)) / Decimal("100"))), 2))
        })
        sync_save_db(); st.rerun()
    ...
    if details:
        ...
        st.divider(); st.markdown("### 📊 各分类成本总计")
        ...
        if st.button("💾 确认并保存修改", type="primary"):
            for idx, row in edited_df.iterrows():
                try:
                    qty_d = Decimal(str(row.get('数量', 1.0))); unit_d = Decimal(str(row.get('税后单价', 0.0)))
                    tax_str = str(row.get('税点', '0%')).replace('%', ''); rate_d = Decimal(tax_str) if tax_str else Decimal("0.0")
                    tot_d = qty_d * unit_d; tax_div = Decimal("1") + (rate_d / Decimal("100"))
                    edited_df.at[idx, '税后总成本'] = float(tot_d); edited_df.at[idx, '税前总成本'] = float(round(tot_d / tax_div, 2))
                except: pass
        ...
        st.info(f"**💰 累计税后总成本:** ¥{total_c:,.2f} | **单体核算成本:** ¥{unit_c:,.2f} | **单体毛利:** ¥{saved_price - unit_c:,.2f} | **预测毛利率:** {(saved_price - unit_c) / saved_price * 100 if saved_price > 0 else 0:.2f}%")
# ==========================================
# 模块 6：历史溯源
# ==========================================
    ...
    grouped_logs = {}; log_ref_map = {}
    for c_name, comp in db[sel_proj].get("部件列表", {}).items():
        if sel_comp != "🌐 全部展示" and c_name != sel_comp: continue
        for log in comp.get("日志流", []):
            log_ref_map[log["_id"]] = log
            key = (log.get("日期",""), log.get("工序",""), log.get("流转",""), log.get("事件",""), str(log.get("图片",[])))
            if key not in grouped_logs: grouped_logs[key] = {"_ids": [log["_id"]], "部件": [c_name], "log": log}
            else: grouped_logs[key]["_ids"].append(log["_id"]); grouped_logs[key]["部件"].append(c_name)

    flat_data = []
    for g in grouped_logs.values():
        flat_data.append({"_ids": g["_ids"], "部件": ", ".join(g["部件"]), "日期": g["log"]["日期"], "工序": g["log"]["工序"], "类型": g["log"]["流转"], "事件": g["log"]["事件"], "图片": g["log"].get("图片", [])})
    ...
    if flat_data:
        ...
        edited_df = st.data_editor(
            ...
            column_config={"序号": st.column_config.NumberColumn(disabled=True), "部件": st.column_config.TextColumn(disabled=True), "工序": st.column_config.SelectboxColumn("工序", options=STAGES_UNIFIED, required=True)},
        ...
        if st.button("💾 确认并覆盖保存历史记录", type="primary"):
            ...
            for i, row in edited_df.iterrows():
                if pd.isna(row.get("部件")) or str(row.get("部件")).strip() in ["", "None", "nan"]: continue
                ...
                for c in c_list:
                    ...
                    if not isinstance(old_images, list): old_images = [old_images] if old_images else []

                    new_logs_by_comp[c].append({"_id": str(uuid.uuid4()), "日期": str(row["日期"]), "工序": str(row["工序"]), "流转": str(row["类型"]), "事件": str(row["事件"]), "图片": old_images})

            comps_in_scope = [sel_comp] if sel_comp != "🌐 全部展示" else list(db[sel_proj].get("部件列表", {}).keys())
            for c in comps_in_scope:
                if c in db[sel_proj].get("部件列表", {}): db[sel_proj]["部件列表"][c]["日志流"] = sorted(new_logs_by_comp.get(c, []), key=lambda x: x.get("日期", ""))
            sync_save_db(); st.success("✅ 历史记录已更新！"); st.rerun()

        st.divider(); st.subheader("🖼️ 历史参考图画廊 (时间倒序)")
        has_images = False
        ...
        for g_data in flat_data:
            ...
            if images:
                ...
                if clean_detail: st.caption(f"📝 {clean_detail}")

                cols = st.columns(6)
                for i, img_b64 in enumerate(images):
                    with cols[i % 6]: render_image(img_b64, use_container_width=True)
                st.markdown("---")
            ...
            if st.button("🚨 确认全库替换", type="primary") and old_n:
                ...
                for p_data in db.values():
                    ...
                    for c_data in p_data['部件列表'].values():
                        ...
                        if changed:
                            c_data['负责人'] = ", ".join(new_pairs); count_fixed += 1
                sync_save_db(); st.success(f"✅ 清洗完成！全库共修正 {count_fixed} 处记录。"); st.rerun()

    with st.expander("⏱️ 全局计划排期默认基线"):
        st.info("设定当建立新项目时，各阶段的默认目标天数。")
        ...
        if st.button("💾 保存默认基线天数"):
            ...
            sync_save_db(); st.success("已更新默认计划基线！"); st.rerun()

    with st.expander("🧹 数据库瘦身 & Base64 图片迁移工具", expanded=True):
        st.warning("⚠️ 如果你的 JSON 文件很大，说明旧版 Base64 图片还留在数据库里。点击下方按钮一键迁移到本地文件，JSON 将大幅缩小。")
                ...
                for log in c_data.get("日志流", []):
                    for img in log.get("图片", []):
                        if isinstance(img, str):
                            ...
                            elif len(img) > 100: b64_count += 1  # 是 base64
        ...
        if b64_count > 0:
            if st.button("🚀 一键迁移：将所有 Base64 图片转存为本地文件", type="primary"):
                ...
                st.success(f"🎉 迁移完成！成功 {migrated} 张，失败 {errors} 张。JSON 从 {json_size_mb:.1f}MB 缩减至 {new_size_mb:.1f}MB，节省了 {saved_mb:.1f}MB！")
                st.rerun()
        else:
        ...
        st.caption("如果 img_assets 目录还是很大，可以对所有本地图片进行二次压缩。")
        col_q1, col_q2 = st.columns(2)
        with col_q1: recomp_quality = st.slider("压缩质量", 30, 85, 60, help="数值越低文件越小，画质越差")
...
# ==========================================
    ...
    st.title("📖 INART PM 系统 (v47) 核心操作指南")
    st.info("👋 欢迎使用全新升级的 INART 产品研发追踪管控系统！本系统专为解决制造业并行研发、跨部门协作痛点而生。")
    st.markdown("---")

    with st.expander("🎯 核心场景 1：日常进度交接与流转", expanded=True):
        st.markdown("1. **进入专属操作台**：点击左侧 **【🎯 特定项目管控台】**，上方选择项目。\n2. **填写【基础信息】与【细分角色】**：根据进度选择更新阶段并填入成员名称。\n3. **填入进展详情**与图片。\n4. 点击最下方的批量保存按钮，**系统会在保存后全自动为你清空表单！**")

    with st.expander("🖼️ 核心场景 2：本地化无感压缩与免按钮粘贴", expanded=True):
        st.markdown("系统内置 **无感画质压缩引擎**，丢进系统的原图会被瞬间压缩保证网页流畅！所有图片现已保存至本地 `img_assets` 目录，告别卡顿。\n**原生闪电粘贴**：千万别用鼠标去点虚线框！点击网页里的【任意空白背景处】（让网页获得焦点），直接按 `Ctrl+V`，图片就会丝滑飞入虚线框！")

    with st.expander("📊 核心场景 3：单轨智能进度大盘", expanded=False):
        st.markdown("1. **【单轨进度条】**：按真实发生时间追踪进度。\n2. **发货区间标签**：项目名旁边会带上一个 `📦[2026 Q2]` 样式的标签。\n3. **🌟 亮金红星**：如果项目录入了【预计开定时间】，甘特图上会亮起一颗非常显眼的金色星星！")
