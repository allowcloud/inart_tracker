...
STD_MILESTONES = ["待立项", "研发中", "待开定", "已开定", "即将进入生产", "生产中", "生产结束", "项目结束撒花🎉"]
STD_COMPONENTS = ["头雕(表情)", "素体", "手型", "服装", "配件", "地台", "包装"]
STAGES_UNIFIED = ["立项", "建模(含打印/签样)", "涂装", "设计", "手板/结构板", "官图", "工厂复样(含胶件/上色等)", "大货", "✅ 已完成(结束)"]
...
def get_macro_phase(detail_stage):
    ...
    if any(x in s for x in ["生产", "大货", "量产", "开定"]): return "生产"
...
# 🚀 【神级功能】云端防丢备份与恢复机制
st.sidebar.divider()
st.sidebar.markdown("### ⚙️ 数据备份与恢复 (防丢失)")
st.sidebar.caption("💡 每日下班前点击下载备份。若云端数据被清空，上传备份即可瞬间恢复！")
...
try:
    ...
    st.sidebar.download_button(
        label="💾 下载最新系统数据 (备份)",
        data=json_bytes,
        file_name=f"inart_pm_backup_{datetime.date.today()}.json",
        mime="application/json"
    )
except:
    st.sidebar.warning("暂无数据可备份，请先录入项目。")

# 提供上传 JSON 文件的恢复按钮
restore_file = st.sidebar.file_uploader("📂 上传备份文件以恢复", type=['json'])
if restore_file is not None:
    if st.sidebar.button("⚠️ 确认覆盖恢复数据", type="primary"):
        try:
            restored_data = json.load(restore_file)
            db_manager.save(restored_data)
            st.session_state.db = restored_data
            st.sidebar.success("🎉 恢复成功！系统满血复活！请手动刷新网页！")
        except Exception as e:
            st.sidebar.error("文件格式错误，恢复失败！")
...
if menu == MENU_DASHBOARD:
    ...
    for proj in valid_projs:
        ...
        if not comps:
            table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "Milestone": ms, "开定时间": tgt, "部件": "-", "流程": "-", "断更": "-", "最新动态": "无数据"})
        else:
            for c_name, info in comps.items():
                ...
                if logs:
                    try:
                        ...
                        off = (datetime.date.today() - last_dt).days; dt_txt = f"{off} 天"
                    except: dt_txt = "未知"
                    l_log = logs[-1]['事件']
                else: dt_txt = "-"; l_log = "无数据"
                table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "Milestone": ms, "开定时间": tgt, "部件": c_name, "流程": stage, "断更": dt_txt, "最新动态": l_log})

    for proj in valid_projs:
        all_logs = []
        for c_name, info in db[proj].get('部件列表', {}).items():
    ...
    if uploaded_csv is not None and st.button("开始智能合并"):
        ...
        for i, row in df.iterrows():
            if '项目名称' in [str(x).strip() for x in row.values]: h_idx = i; break
        if h_idx != -1:
            ...
            if '负责人' in df.columns:
                ...
                for _, r in df[df['负责人'].notna()].iterrows(): 
                    ...
                    cnt += 1
                sync_save_db(); st.success(f"同步了 {cnt} 条数据！权限已刷新！"); st.rerun()
    ...
    with c1:
        if owner_stats:
            ...
            st.plotly_chart(px.bar(df_owner, x='人员', y='积压任务数', title="👤 团队当前负荷", color='积压任务数', color_continuous_scale='Reds'), use_container_width=True)
    with c2:
        if stage_stats:
...
# ==========================================
    ...
    else: st.info("暂无足够历史日志以生成时长漏斗。请在下方【细分配件状态推进】中录入进度即可激活。")
    ...
    if not comps: st.warning("暂无录入部件明细。请在下方录入。")
        ...
        for comp_name in y_labels:
            ...
            for i in range(len(STAGES_UNIFIED)):
                if STAGES_UNIFIED[i] == "✅ 已完成(结束)" and cur_stage == "✅ 已完成(结束)":
                    row_vals.append(1); row_hover.append(f"部件: {comp_name}<br>状态: ✅ 全部结束")
                elif i < c_idx: row_vals.append(1); row_hover.append(f"部件: {comp_name}<br>工序: {STAGES_UNIFIED[i]}<br>状态: ✅ 已完成")
                elif i == c_idx and cur_stage != "✅ 已完成(结束)": row_vals.append(2); row_hover.append(f"部件: {comp_name}<br>工序: {STAGES_UNIFIED[i]}<br>状态: 🚀 <b>当前进行中</b>")
                else: row_vals.append(0); row_hover.append(f"部件: {comp_name}<br>工序: {STAGES_UNIFIED[i]}<br>状态: ⏳ 未开始")
    ...
    cur_macro = db[sel_proj].get('宏观大阶段', '立项')
    ...
    with col_m2: new_ms = st.selectbox("修改细节 Milestone", STD_MILESTONES, index=STD_MILESTONES.index(cur_ms) if cur_ms in STD_MILESTONES else 0)
    with col_m3: new_target = st.text_input("📅 预计开定时间", value=cur_target)

    st.markdown("**2. ⚡ 阶段切入与历史补录 (非必填)**")
    st.caption("💡 提示：如果只需修改基础信息，可跳过此项。如果有需要【强制切入新阶段】或【补录历史节点日期】的才需要在这里进行更改。")
    col_opt1, col_opt2 = st.columns([1.5, 1.5])
    with col_opt1: new_macro = st.selectbox("🎯 强制切入宏观阶段", ["-- 不更改 --", "立项", "建模", "设计", "工程", "生产", "结束"], index=0)
    with col_opt2: global_record_date = st.date_input("🕒 发生日期 (若为补录请在此回溯)", datetime.date.today(), key="date_global")

    if st.button("💾 更新全局状态", type="primary", key="btn_global"): 
        db[sel_proj]['负责人'] = new_pm 
        db[sel_proj]['Milestone'] = new_ms
        db[sel_proj]['Target'] = new_target 

        is_end_milestone = (new_ms == "项目结束撒花🎉")
        td = str(global_record_date)
        comps_list = list(db[sel_proj].get('部件列表', {}).keys())
        t_c = "全局进度" if "全局进度" in comps_list else (comps_list[0] if comps_list else "全局进度")

        if t_c not in db[sel_proj].setdefault("部件列表", {}): db[sel_proj]["部件列表"][t_c] = {"主流程": cur_macro, "日志流": []}

        if new_macro != "-- 不更改 --" or is_end_milestone:
            actual_macro = "结束" if is_end_milestone else new_macro
            db[sel_proj]['宏观大阶段'] = actual_macro
            db[sel_proj]["部件列表"][t_c]["主流程"] = actual_macro
            ...
            db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": actual_macro, "事件": f"[全局同步] 大盘切入:{actual_macro} | 里程碑:{new_ms}"})
        else:
            db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": cur_macro, "事件": f"[属性更新] Milestone:{new_ms} | 开定:{new_target}"})

        sync_save_db(); st.success(f"已更新大盘配置！"); st.rerun()

    st.markdown("**3. 细分配件状态推进**")
    existing_comps = list(db[sel_proj].get('部件列表', {}).keys())
    custom_comps = sorted([c for c in existing_comps if c not in STD_COMPONENTS])
    all_comps = STD_COMPONENTS + custom_comps
    all_comps.insert(0, "➕ 新增细分配件...") 

    m1, m2 = st.columns([1, 2])
    with m1:
        sel_comp_raw = st.selectbox("操作部件", all_comps)
        if sel_comp_raw == "➕ 新增细分配件...":
            sub_cat = st.selectbox("所属主分类", STD_COMPONENTS)
            sub_name = st.text_input("细分名称 (如: 持枪手, 电锯)", placeholder="输入具体名称")
            sel_comp = f"{sub_cat} - {sub_name}" if sub_name else ""
        else: sel_comp = sel_comp_raw

        cur_stage = db[sel_proj].get('部件列表', {}).get(sel_comp, {}).get('主流程', STAGES_UNIFIED[0]) if sel_comp else STAGES_UNIFIED[0]
        new_stage = st.selectbox(f"目标工序/当前所处阶段", STAGES_UNIFIED, index=STAGES_UNIFIED.index(cur_stage) if cur_stage in STAGES_UNIFIED else 0)
        p_owner = st.text_input("部件负责人", value=db[sel_proj].get('部件列表', {}).get(sel_comp, {}).get('负责人', '') if sel_comp else '')
    ...
    with m2:
        ...
        log_txt = st.text_area("补充详细进展", height=80)

        if st.button("💾 保存交接与进度", type="primary", key="btn_detail"):
            if not sel_comp or not str(sel_comp).strip(): st.error("❌ 配件名称不能为空！")
            else:
                sel_comp = str(sel_comp).strip()
                if sel_comp not in db[sel_proj]["部件列表"]: db[sel_proj]["部件列表"][sel_comp] = {"主流程": new_stage, "日志流": []}
                db[sel_proj]["部件列表"][sel_comp]['主流程'] = new_stage
                db[sel_proj]["部件列表"][sel_comp]['负责人'] = p_owner

                full_log = f"【{evt_type} | {handoff}】状态:{reject}。补充: {log_txt}" if log_txt else f"【{evt_type} | {handoff}】状态:{reject}。"
                db[sel_proj]["部件列表"][sel_comp]['日志流'].append({"日期": str(detail_record_date), "流转": evt_type, "工序": new_stage, "事件": full_log})
                sync_save_db(); st.success(f"🎉 【{sel_comp}】 的进度已记录至 {detail_record_date}！"); st.rerun()
...
# ==========================================
    ...
    raw_text = st.text_area("✍️ 输入进展 (如: 里夫签样OK)：", height=150)
    ...
    if st.session_state.parsed_logs:
        ...
        for i, item in enumerate(st.session_state.parsed_logs):
            ...
            with c1: sel_proj = st.selectbox(f"归属项目 {i+1} (💡打字可搜索)", project_options, index=project_options.index(item['识别项目']) if item['识别项目'] in project_options else 0, key=f"sel_{i}")
        ...
        if st.button("💾 确认无误，全部入库！", type="primary"):
            ...
            for log in edited_logs:
                ...
                db[p].setdefault("部件列表", {}).setdefault(t_c, {"主流程": curr_actual_stage, "日志流": []})['日志流'].append({"日期": td, "流转": "AI速记", "工序": curr_actual_stage, "事件": log['事件']})
            sync_save_db(); st.session_state.parsed_logs = []; st.success("🎉 入库成功！"); st.rerun()
