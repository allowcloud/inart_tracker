...
* ✅ **多管线并发**：`1/6萨鲁曼：头雕打样中；目标定价2980；法杖需要修改` -> 自动拆分为 3 笔独立记录
    ...
    if st.button("✨ 智能拆解", type="primary"):
        if not raw_text.strip():
            ...
            parsed = []
            prefix_pattern = re.compile(r'^(1/6|1/4|1/12|1/3|1/1)')
            for line in [l.strip() for l in raw_text.split('\n') if l.strip()]:
                ...
                for rp in raw_projs:
                    ...
                    elif current_prefix and not prefix_pattern.search(rp):
                        rp = f"{current_prefix} {rp}".replace("  ", " ")
                    matched_p = next(
                        (vp for vp in valid_projs
                         if rp.replace(" ", "").lower() in vp.replace(" ", "").lower()),
                        "未知/请手动修改"
                    )
                    if matched_p == "未知/请手动修改":
                        core_raw  = re.sub(r'^(1/6|1/4|1/12|1/3|1/1)\s*', '', rp).strip()
                        matched_p = next(
                            (vp for vp in valid_projs if core_raw and core_raw in vp),
                            "未知/请手动修改"
                        )
                ...
                for up in [u.strip() for u in re.split(r'[;；]', updates_part) if u.strip()]:
                    ...
                    for rp in resolved_projs:
                        parsed.append({"识别项目": rp, "推测部件": detected_comp,
                                       "推测阶段": detected_stage, "待写入事件": up})
