...
import re      # 🚀 修复补丁 1：新增正则表达式库，解决大盘 loading 解析报错
import uuid    # 🚀 修复补丁 2：新增 uuid 库，解决历史溯源模块无 ID 时的崩溃问题
import io      # 🚀 优化补丁：用于内存转换剪贴板图片
...
def auto_sync_milestone(proj_name):
    ...
    # 🚀 补丁 1：领头羊效应 -> 子部件进度倒逼全局进度自动升级
    if sub_stages:
        max_idx = -1
        for s in sub_stages:
            if s in STAGES_UNIFIED:
                idx = STAGES_UNIFIED.index(s)
                if idx > max_idx: max_idx = idx

        if max_idx >= 0:
            max_stage = STAGES_UNIFIED[max_idx]
            global_keys = [k for k in comps.keys() if "全局" in k]
            global_key = global_keys[0] if global_keys else "全局进度"

            if global_key not in comps:
                comps[global_key] = {"主流程": max_stage, "日志流": []}
            else:
                curr_global_stage = comps[global_key].get("主流程", STAGES_UNIFIED[0])
                # 如果全局当前阶段落后于子部件最高阶段，则自动升级全局进度
                if curr_global_stage in STAGES_UNIFIED and STAGES_UNIFIED.index(curr_global_stage) < max_idx:
                    comps[global_key]["主流程"] = max_stage
                    comps[global_key].setdefault("日志流", []).append({
                        "日期": str(datetime.date.today()), "流转": "系统自动", "工序": max_stage,
                        "事件": f"[系统自动追踪] 因子部件到达【{max_stage}】，全局进度自动同步升级"
                    })
...
def sync_save_db():
    ...
    # 🚀 架构优化补丁：极简防并发机制 (并发合并读写)
    # 每次写盘前，先拉取硬盘最新数据，仅用当前会话的数据进行增量覆盖，防止抹除其他 PM 的修改
    ...
    st.session_state.db = latest_db  # 刷新当前会话引用
...
# ==========================================
    ...
    with m1:
        ...
        # 🚀 终极体验：自动提取全库历史人名，构建带模糊搜索的智能下拉框
        st.markdown("**👥 细分角色分配 (💡 支持打字模糊搜索提示)**")

        # 1. 动态提取所有项目中的历史人名建立记忆词库
        ...
        for idx, r in enumerate(role_list):
            with r_cols[idx%3]:
                ...
                # 动态适配：如果原名字不在常规词库里，临时加进下拉选项以防丢失
                ...
                # 使用 selectbox 即可原生支持点击后的键盘打字模糊过滤
        ...
        # 初始化会话缓存，用于存储多次粘贴的图片和被剔除的图片
        if 'pasted_cache' not in st.session_state: st.session_state.pasted_cache = {}
        if 'exclude_imgs' not in st.session_state: st.session_state.exclude_imgs = set()
        ...
        try:
            ...
            st.warning("💡 请终端运行: pip install streamlit-paste-button")
            paste_result = None
        ...
        if st.button("💾 批量保存交接与进度", type="primary", key="btn_detail"):
            if "➕ 新增细分配件..." in comps_to_process and not new_comp_name: 
                ...
                # 🚀 优化：从聚合预览池（已剔除垃圾桶图片）生成 Base64 入库
                img_b64_list = []
                ...
                for c_raw in comps_to_process:
                 ...
                 # ⚠️ 极其关键：保存完毕后，立即重置粘贴缓存与剔除名单，避免污染下一次交接
        ...
        if st.button("💾 确认并覆盖保存历史记录", type="primary"):
            ...
            for _, row in edited_df.iterrows():
                ...
                new_logs_by_comp[c].append({
                    "_id": str(uuid.uuid4()), "日期": str(row["日期"]), "工序": str(row["工序"]),
