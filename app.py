...
class DatabaseManager:
    ...
    def save(self, data):
        import tempfile
        # 🚀 架构级修复：采用原子写入 (Atomic Write) 防断电/并发导致的数据清零
...
def auto_sync_milestone(proj_name):
    ...
    for c_name, info in comps.items():
        if "全局" in c_name: continue  # 排除全局
        s = str(info.get('主流程', '')).strip()

        # 🚀 强化补丁：支持阶段的简写模糊匹配 (如 "建模" 也能识别为 "建模(含打印/签样)")
        s_idx = -1
    ...
    if max_idx >= 0 and max_stage:
        ...
        # 🚀 核心逻辑：如果全局当前阶段落后（或未识别=-1），立刻启动升级引擎！
        if curr_idx < max_idx:
            # 完美级体验：不仅拉平当前阶段，还会自动把跳过的中间阶段补齐，让矩阵图的进度格子连贯不断层！
...
# ==========================================
    ...
    if st.button("入账"):
        ...
        # 🚀 架构级修复：使用高精度 Decimal 进行财务级运算，彻底消灭浮点误差
        ...
        db[sel_proj].setdefault("成本数据", {}).setdefault("动态明细", []).append({
            ...
            "税后单价": float(d_unit), "数量": float(d_qty),
            "税后总成本": float(tot_after), "税点": f"{tax_rate}%", 
            "税前总成本": float(round(pre_tax, 2))
        })
        sync_save_db(); st.rerun()
