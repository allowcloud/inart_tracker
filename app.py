...
if menu == MENU_DASHBOARD:
    ...
    for proj in valid_projs:
        ...
        if not comps:
            table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "当前阶段": ms, "开定时间": tgt, "断更": "-", "最新全盘动态": "无数据"})
        else:
            ...
            table_data.append({"状态": r_txt, "项目": proj, "跟单": gd, "当前阶段": ms, "开定时间": tgt, "断更": dt_txt, "最新全盘动态": final_event})
...
# ==========================================
        ...
        for comp_name in y_labels:
            ...
            for i in range(len(STAGES_UNIFIED)):
                ...
                if cur_stage == "✅ 已完成(结束)":
                    ...
                    is_forced_completed = (c_idx >= guan_tu_idx and i < guan_tu_idx)
    ...
    with col_m2: new_ms = st.selectbox("当前阶段", STD_MILESTONES, index=STD_MILESTONES.index(cur_ms) if cur_ms in STD_MILESTONES else 0)
    with col_m3: new_target = st.text_input("📅 预计开定时间", value=cur_target)

    st.markdown("**2. ⚡ 阶段切入与历史补录 (如有需要才更改)**")
    col_opt1, col_opt2 = st.columns([1.5, 1.5])
    # 🚀 优化 2：隔离文案，这里是“大盘工序”
    with col_opt1: new_macro = st.selectbox("🌍 全局大盘工序切入", ["-- 不更改 --"] + STAGES_UNIFIED, index=0)
    ...
    if st.button("💾 更新全局状态", type="primary", key="btn_global"): 
        ...
        if new_macro != "-- 不更改 --" or is_end_milestone:
            actual_macro = "✅ 已完成(结束)" if is_end_milestone else new_macro
            db[sel_proj]['宏观大阶段'] = get_macro_phase(actual_macro)
            db[sel_proj]["部件列表"][t_c]["主流程"] = actual_macro

            if actual_macro == "✅ 已完成(结束)":
            ...
            if new_macro == "立项":
                db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": "立项", "事件": f"[全局同步] 补录立项"})
                next_day = str(global_record_date + datetime.timedelta(days=1))
                db[sel_proj]["部件列表"][t_c]['主流程'] = "建模(含打印/签样)"
                db[sel_proj]['宏观大阶段'] = "建模"
                db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": next_day, "流转": "系统更新", "工序": "建模(含打印/签样)", "事件": f"[系统自动] 立项完成，自动进入建模阶段"})
                st.success(f"已补录立项！系统已自动将次日进度切入【建模】。")
            else:
                db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": actual_macro, "事件": f"[全局同步] 大盘切入:{actual_macro} | 里程碑:{new_ms}"})
                st.success(f"已更新大盘配置！")
        else:
            db[sel_proj]["部件列表"][t_c]['日志流'].append({"日期": td, "流转": "系统更新", "工序": cur_macro_state, "事件": f"[属性更新] 当前阶段:{new_ms} | 开定:{new_target}"})
            st.success(f"已更新大盘基础信息！")

        sync_save_db(); st.rerun()

    st.markdown("**3. 细分配件状态与全局日志 (💡支持批量多选推进)**")
    ...
    with st.expander("📥 批量导入产品配件清单 (CSV)"):
        ...
        if comp_csv and st.button("开始识别并导入配件"):
            try:
                ...
                name_col = next((c for c in df_c.columns if any(k in str(c) for k in ['名称', '部件', '零件', '配件', '明细'])), None)
                if name_col:
                    count = 0
    ...
    with m1:
        ...
        # 🚀 优化 2：明确文案“部件工序阶段”，且默认值自动感应所选部件状态！
        ...
        if st.button("💾 批量保存交接与进度", type="primary", key="btn_detail"):
            if "➕ 新增细分配件..." in comps_to_process and not new_comp_name: 
                ...
                for c_raw in comps_to_process:
                    ...
                    if actual_c not in db[sel_proj]["部件列表"]: db[sel_proj]["部件列表"][actual_c] = {"主流程": new_stage, "日志流": []}
                    ...
                    if img_file is not None:
                        img_file.seek(0)
                        log_entry["图片"] = base64.b64encode(img_file.read()).decode()
